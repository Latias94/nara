...
use nara::{
    ...
    ecs::{Entity, With, Without, error::BevyError, system::SystemParam},
...
use crate::{
    ...
    resources::{
        ...
        MOVE_COMMAND_TYPE, MOVE_X_FIELD, MOVE_Y_FIELD, PLAYER_SCENE_ID, PROJECTILE_TTL_TICKS,
        ProjectileId, WaveState,
    },
...
pub(crate) fn begin_wave_tick(mut state: ResMut<WaveState>) {
    state.begin_tick();
}
...
pub(crate) fn pursue_scene_players(
    ...
    let result = (|| {
        for (enemy_source, spawn, mut enemy) in &mut enemies {
            if enemy.hit_points <= 0 || spawn.tick > fixed_time.tick() {
                continue;
            }
...
pub(crate) fn fire_automatic_weapons(
    ...
    let result = (|| {
        ...
        let projectile_id = state
            .allocate_projectile_id()
            .ok_or_else(|| BevyError::error(ReferenceSimulationError::ProjectileIdentityOverflow))?;
        commands.spawn((
            Projectile {
...
pub(crate) fn resolve_wave_projectile_hits(
    ...
    let result = (|| {
        let mut target_order = enemies
            .iter()
            .filter(|(_, _, spawn, enemy)| enemy.hit_points > 0 && spawn.tick <= fixed_time.tick())
...
fn retire_expired_entities_inner(world: &mut World) -> Result<(), BevyError> {
    ...
    dead_enemies.sort_by(|left, right| {
        left.0
            .cmp(&right.0)
...
pub(crate) struct ProjectSnapshotQueries<'w, 's> {
    players: Query<'w, 's, (&'static SceneEntitySource, &'static Player, Option<&'static Weapon>)>,
