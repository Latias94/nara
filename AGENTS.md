...
- `nara_gameplay` owns gameplay command envelopes, stable command target vocabulary, action-to-command mapping, and `GameplayCommandQueue`. It must not introduce networking transports or runtime `Entity` values into durable command data.
