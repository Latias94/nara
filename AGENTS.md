...
- Play Mode must use an isolated runtime `World` fork spawned from a validated edit document snapshot. Stop Play discards runtime changes by default; persistent write-back must be an explicit Apply Changes flow that produces `ScenePatchDocument` and goes through normal validation/undo.
