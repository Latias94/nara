...
- nara is an integrated product built from explicit modules. The first-party default combination prioritizes a coherent workflow; crate and plugin boundaries support documented reuse and replacement without promising arbitrary cross-engine compatibility.
...
- Scripting runtimes are optional plugin/package adapters, not a required second author language. Do not freeze a universal Behavior Host, default Wasm ABI, or core VM dependency before a concrete adapter and game workflow prove the shared contract.
...
- Do not freeze a public service/backend Adapter from hypothetical implementations or mocks. Require production-shaped variation pressure plus an independent implementation challenge, and a second real Adapter before compatibility freeze; fakes prove conformance and faults only.
