...
- R13. A default plugin group must expose stable slot IDs for disabling, same-plugin configuration, and relative ordering before installation; slot identity remains separate from installed plugin identity. Cross-plugin replacement and slot-contract version compatibility remain trigger backlog until a second concrete `PluginId` and public conformance suite prove them, and invalid pure composition leaves the `App` reusable for a later valid attempt.
...
- R17. Editor Restart must stop the old owner and construct a fresh `App`, `World`, queue, task/backend state, and runtime generation from a profile-specific `RuntimeRecipe`. Headless, desktop, and Editor recipes may share one immutable `ProjectContentSnapshot` and catalog/binding revision but must own separately resolved runtime plans; a structural schema or authoring-document change creates a new snapshot/recipe revision. Desktop Retry is a game-owned run reset inside the existing runtime, while standalone structural edits use process restart.
...
- R31. Runtime construction must register each plugin and service attempt before invoking a fallible build, finish, or acquisition hook. First-party hooks must remain nonblocking; waitable admission and shutdown work is candidate-owned, pollable, cancellable, and deadline-bound. A mid-hook failure or Host cancellation must retire the unpublished candidate without publishing a partially owned `App` or `RuntimeInstance`.
...
- R22. The first instrumented slice must record P50/P95 edit-to-result latency for data, compatible Rust function-body, and structural Rust changes; clean checkout-to-headless-wave and clean checkout-to-manually-playable-desktop time; add-module and supported-slot-configuration success/time; public production coverage; frame-time P99; memory; build time; and artifact size.
...
- AE17. Given a first-party plugin or service that acquires an owned resource and then fails inside `build` or `finish`, the current attempt already owns that acquisition, every acquired owner is shut down exactly once, and no Running runtime is published. A never-completing waitable admission remains Host-drivable; Stop/cancel moves its `RuntimeStartAttempt` through retirement and deadline-bound `RetirementIncomplete` without blocking the main thread.
...
- Generic staged slots demonstrate same-plugin configuration, disable, and ordering before the headless wave; after the U14 Continue gate, the desktop group configures the existing window slot without core edits. Cross-plugin replacement remains absent until a second implementation proves a separate contract.
...
7. **Adopt configurable groups only as consumers arrive.** Nara keeps its fallible `App` and closed core lifecycle. U4 introduces stable slot identity, same-plugin configuration, disable, add-before, and add-after because the headless/desktop products consume them. It also opens typed custom-schedule registration while leaving custom schedules inert unless an explicit owner runs them. Cross-plugin replacement, slot-contract version compatibility, and automatic main-loop insertion remain deferred until concrete consumers and conformance evidence exist.
...
10. **Separate content, profile policy, and runtime-start ownership.** The root project Host privately owns an immutable `ProjectContentSnapshot`; it combines that snapshot with a separately resolved headless, desktop, or Editor `RuntimePlan` to form a private profile-specific `RuntimeRecipe`. A concrete Host's `begin_start` returns one `RuntimeStartAttempt` immediately and retains/drives it until it yields a published `RuntimeInstance` or retains `RetirementIncomplete` ownership. Do not freeze a public object-safe `RuntimeFactory` before two genuinely substitutable producers exist. Tooling never imports root-private snapshot/recipe types and never owns the start attempt or runtime instance; it receives only commands and views.
11. **Make shutdown cooperative and qualify fatal faults.** First-party synchronous plugin hooks are audited non-blocking code, not preemptible runtime work. Services that may wait expose pollable completion and a deadline; the runtime drives them at safe points and retains `RetirementIncomplete` or `CloseIncomplete` on timeout. A typed fatal-runtime-fault channel accepts gameplay lifecycle invariants, fallible systems, and explicitly required task/service integrations; optional task failures and last-good-preserving asset reload failures remain domain diagnostics.
...
15. **Keep file authority in a concrete product host without freezing a universal host API.** The root product integration owns the first file-backed content snapshot, private profile-specific recipes, and process-lifetime capabilities; `nara_project`, `nara_asset`, and `nara_scene` keep parsing, import, and spawn semantics in their domains. Extract another public crate only if two real hosts require the same boundary.
...
20. **Keep ADR decision status separate from implementation evidence.** `Accepted` means Nara selected a decision; `implemented` in the ledger means code and verification prove it. High-cost candidates remain Proposed while named plan units run bounded evidence trials. U5/U17/U13 do not silently admit ADRs 0082/0084: U23 performs a separate atomic joint admission review after all three evidence paths and blocks Publish while either proposal remains non-authoritative. Adding `Trial` to the ADR decision enum would conflate authority with execution progress, so U19 strengthens set/link/anchor validation without redefining that lifecycle.
21. **Bind each runtime recipe to one immutable content and authoring revision.** Runtime attempts consume the exact bounded documents, imported artifact values, and digests in their shared `ProjectContentSnapshot`; they do not reopen project source. Profile-specific recipes differ only in validated runtime policy such as the `RuntimePlan`. Source or authoring-document changes create a new snapshot/recipe revision, and the concrete Host rejects stale recipes before Start/Restart.
...
flowchart TD
    ...
    Profile[Resolved headless, desktop, or Editor RuntimePlan] --> Recipe
    Recipe --> Start[Host-owned RuntimeStartAttempt]
    Start --> Prepare[Authority-free plugin preparation]
    Prepare --> Reserve[Attempt-owned guarded inactive reservations]
    Host -->|issues through domain Adapters| Reserve
    Reserve --> Candidate[Private unpublished RuntimeCandidate]
...
flowchart LR
    ...
    Scene --> Snapshot
    Candidate --> Snapshot
    Snapshot --> Recipe[Profile-specific RuntimeRecipe]
    Profile[Resolved RuntimePlan and authoring revision] --> Recipe
    Recipe --> Attempt[Fresh RuntimeStartAttempt from exact immutable values]
...
`Starting`, `Retiring`, and `RetirementIncomplete` belong to the Host-owned
...
After the old instance reaches `Stopped`, the concrete Host may begin a distinct fresh attempt. A
new `RuntimeInstance` becomes observable only when admission publishes `Running`; this is not a
...
- The Editor workspace supports at most one active Play runtime in this slice. Closing, reloading, restarting, or exiting while Play owns resources must stop it first; shutdown failure retains the owner and a CloseIncomplete diagnostic instead of silently dropping it.
...
- File-backed project boot becomes a root-product host workflow over opaque `nara_fs` authority, a byte-and-count-bounded immutable `ProjectContentSnapshot`, and profile-specific runtime recipes rather than ambient reads or one universal plugin plan.
...
- Plugin groups become staged and configurable before installation; inspection exposes slot identity, same-plugin definition/config identity, and actual plugin identity separately.
...
- Potentially blocking admission and shutdown move from synchronous plugin-hook assumptions to candidate-owned pollable work with cancellation and deadlines.
...
| A failed runtime start leaks threads, watchers, or partially admitted services | Make `RuntimeStartAttempt` retain the unpublished candidate and pollable shutdown ownership through Starting, Retiring, and RetirementIncomplete until completion or process teardown. |
...
| M3 Composable headless runtime | U4, U12, U5 | The tracer uses generic staged slots, one byte-bounded content snapshot, a headless-profile RuntimeRecipe, and a Host-driven RuntimeStartAttempt; exact step and every construction/runtime fault are observable | Composition mutates before validation, an attempt reopens source, aggregate residency escapes its budget, a hook blocks the host, or runtime hosting duplicates schedule authority |
...
| M9 Runtime ownership admission | U23 | An independent metric-by-metric review atomically accepts both ADR 0082/0084 proposals and aligns every authority document | Evidence is incomplete, one proposal differs in status, or Accepted/foundation text claims the model before the atomic decision |
| M10 Final evidence | U20 | Pre-publication review, immutable lineage, clean-room authoring, and exact-final-candidate measurements approve Publish/Redirect/Stop and one next-slice rule | U23 did not Accept Both, affected samples are stale, a frozen range is changed, P0/P1 findings remain, clean-room steps need hidden context, or artifact identity/digest/retention is incomplete |
| M11 Public release | U21 | The final permission-bearing workflow passes independent review; no-checkout write jobs publish exact U20 artifacts around a read-only draft smoke, then anonymous public smoke passes before announcement | Publisher review, tag/version/branch authority, artifact identity/retention, archive preflight, immutable state, or either download smoke fails |
...
- If U23 cannot atomically Accept Both ADR 0082 and ADR 0084, do not enter U20. Reject Both and re-plan, or leave both Proposed while missing evidence is gathered; never publish on a mixed or implicit decision state.
...
| U4 | Headless composition, author ergonomics, and attempt shutdown | `nara_app`, first-party plugin owners, reference game | U3, U10 |
| U12 | Authorized immutable project content snapshot | root product host, project/asset/scene adapters | U3, U10 |
| U5 | Headless runtime recipe, start attempt, and lifecycle | app/gameplay/tasks/root host | U4, U12 |
...
| U23 | Joint runtime-ownership proposal admission | ADR 0082/0084 evidence, authority docs, governance tests | U5, U13, U17, U19 |
| U20 | Pre-publication successor and exact-candidate evidence review | measurement helpers, raw lineage, benchmark review | U7, U8, U13, U14, U17, U19, U23 Accept Both |
...
- **Goal:** Let ordinary Rust authors configure coherent defaults through type-directed edits, retain stable slots for durable/advanced composition, and make every committed first-party plugin attempt truthfully shut down.
...
- **Files:** `crates/nara_app/src/lib.rs` and focused `crates/nara_app/src/{plugin,plugin_group,schedule}.rs` if split, `crates/nara_asset/src/reload.rs`, `crates/nara_asset_watch/src/lib.rs`, `crates/nara_winit/src/lib.rs`, `crates/nara_image/src/lib.rs`, `crates/nara_render_wgpu/src/lib.rs`, `crates/nara_sprite_render/src/lib.rs`, `crates/nara_ui_render/src/lib.rs`, `crates/nara_ui/src/lib.rs`, `src/lib.rs`, `tests/{plugin_composition,product_capabilities,custom_schedule}.rs`, `reference-game/src/{lib,plugins}.rs`, `reference-game/tests/plugin_composition.rs`, ADRs 0003/0010/0046/0079, `docs/architecture/nara-foundation.md`, and migration documentation.
- **Approach:** Introduce stable group and named slot IDs separate from actual `PluginId`; an un-slotted custom entry uses its unique `PluginId` rather than another placement identity. Stable slots remain the durable authority, while ordinary same-plugin configuration/disable/order uses type-directed facades such as `.disable::<TilemapPlugin>()` and `.configure(image::plugin(limits))`; U4 exposes no cross-plugin replacement or slot-contract version API. Replace production nested installation with concise static declarations, stable definition IDs/keys, repeatable typed factories, private entry drafts, and data-only groups; ordinary game/package authors never construct IDs, fingerprints, canonical config, or erased factories. Every build/finish hook rejects `add_plugin`/`add_plugins` before other install logic. Rename terminal `Plugin::cleanup`/contexts/states/errors to `shutdown` vocabulary so it cannot be confused with Bevy's pre-first-update hook or `CoreStage::Cleanup`. Keep one generic `add_systems(ScheduleLabel, ...)`/set-configuration Interface over built-in and custom schedules; custom schedules remain inert unless an explicit owner runs them, and U4 does not open automatic main-loop insertion. The App/Host commit path privately opens an attempt scope that marks each plugin Attempted before invoking `build` or `finish`; audited first-party hooks may perform bounded immediate work but must not wait. Acquired owners register before the next fallible operation, while waitable admission and shutdown register pollable operations with cancellation/deadline state owned by the attempt. A hook error marks the candidate `App` failed/unpublishable and retains it while shutting down the current and prior attempts exactly once; `RetirementIncomplete` retains the scope for later drive of unfinished participants without reinvoking once-only hooks. Synchronous Rust hooks are not preemptible: a watchdog subprocess detects a deliberately blocking conformance fixture, while runtime cancellation guarantees apply only after work has registered as pollable. Cross-plugin replacement and contract versions remain trigger backlog until a second concrete implementation exists.
- **Test scenarios:** Covers AE5 and AE17. Type-directed and stable-slot configuration, disable, ordering, inspection of slot versus plugin identity, image-limit configuration, and gameplay insertion succeed. Clean-room common examples compile without definition IDs, fingerprints, slot constants, plans, or candidate terms. A third-party typed custom schedule accepts systems/sets, runs only when explicitly invoked, and never enters the automatic frame order by registration alone. Duplicate/missing slots, disabled prerequisites, cycles, conflicts, missing services/capabilities, and a configured entry requiring a compiled-but-unrequested capability fail before mutation; a later valid preflight succeeds on the same `App`. Separate fixtures acquire one owner and then fail inside `build`, acquire another and fail inside `finish`, register never-completing admission/shutdown work, and receive cancellation between acquisition polls; the current plugin was already Attempted, every owner shuts down exactly once, the Host thread remains drivable, no candidate publishes, and deadline expiry retains a truthful `RetirementIncomplete` owner. Nested installation rejects during committed build. A deliberately blocking hook times out only inside the watchdog subprocess and is recorded as a first-party contract violation, not a runtime-cancellable path.
- **Verification:** Focused app and every migrated plugin-owner test pass; public headless customization needs no facade edit or stable-infrastructure vocabulary; searches find no plugin/group installation inside any build/finish hook, first-party acquisition outside an attempt scope, wait inside admitted first-party hooks, legacy terminal plugin-cleanup names, or cross-plugin replacement/version surface without a second concrete implementation; group inspection matches resolved slot order and membership; custom schedule registration does not alter automatic order; U3 no longer carries unproved service/conflict/slot closure claims.
...
- **Goal:** Turn one scheduled `App` into a Host-owned start attempt and a reconstructible, fault-propagating, cancellable, truthfully stoppable runtime without depending on Editor or desktop hosts.
...
- **Approach:** Keep one `App` as schedule/world authority and wrap it in a thin lifecycle owner. The concrete root project Host combines U12's immutable snapshot with U4's resolved headless `RuntimePlan` in a private `RuntimeRecipe`; `begin_start(recipe)` returns a `RuntimeStartAttempt` immediately without freezing an object-safe factory trait. The Host retains and drives that attempt through Starting, Retiring, Ready, or RetirementIncomplete; Ready yields the only published `RuntimeInstance`. Each attempt clones immutable content leases and exact digests without duplicating payload bytes, completes authority-free plugin preparation, then requests inactive reservations through concrete Host/domain Adapters before creating the candidate `App` and minting fresh runtime handles, queues, tasks, services, and backend generations. The attempt owns U4's attempt scope, every transferred reservation, registry freeze, native codec binding, final scene preflight/spawn, and pollable admission/shutdown. Host cancellation starts retirement after the current audited synchronous hook returns; deadline guarantees apply only to registered pollable work, and expiry retains the same attempt in RetirementIncomplete for continued drive or process teardown without reinvoking a once-only hook. The published instance begins at Running and uses `Faulted` for sticky execution failure and `CloseIncomplete` for retained shutdown ownership. Exact Step uses a distinct app-frame plan and typed fatal-runtime-fault channel. Local submission, gameplay Admit/Acknowledge, fallible systems, and explicitly required task/service integration failures enter that sticky channel before success. Optional tasks and last-good-preserving asset reload failures remain domain diagnostics and do not fault the runtime.
- **Control matrix:** A Starting attempt accepts drive and one Stop/cancel; duplicate Start/Restart is Busy. Cancel transitions to Retiring and can never yield Ready. RetirementIncomplete accepts drive/retry-retirement or process teardown but cannot yield a runtime. Running accepts Pause and Stop. Paused accepts Resume, one StepFixedTick, and Stop. A second StepFixedTick while Pending or Stepping is Busy. Stepping and Stopping reject unrelated controls; Faulted accepts disposal/Stop only; CloseIncomplete accepts Retry Close; Stopped accepts no runtime-local control. Accepted controls expose Pending then exactly one Applied or Failed result. Pause/Resume/StepFixedTick apply only at the owning safe point, and Stop has priority over a not-yet-applied lower-priority control without reporting that control as Applied.
- **Reconstruction contract:** Two starts from the same recipe revision use identical snapshot content and policy but distinct mutable generations. The old runtime or retirement-incomplete start attempt remains owned until Stopped/retired before a concrete Host publishes a replacement. A new validated snapshot and profile-specific recipe revision is required to observe source, schema, authoring-document, or runtime-plan changes; the runtime core exposes no in-place restart or source reopen.
- **Test scenarios:** Covers AE7, AE17, and AE8's runtime subset. Fixtures acquire and fail inside both plugin `build` and `finish`, then inject failure after freeze, spawn, and required-service admission; no Ready/Running result publishes and every attempted owner shuts down exactly once. A never-completing pollable admission proves cancel reaches Retiring without blocking the Host; a never-completing shutdown reaches RetirementIncomplete without claiming Stopped. Two sequential successful starts and failed-first/retry starts share immutable payload identity and one content charge but use distinct runtime, queue, task, service, and backend generations. Illegal and competing start/runtime controls follow the matrix; duplicate StepFixedTick is Busy; paused frames advance permitted real-time work but no fixed tick. Exact StepFixedTick runs one complete command transaction/tracker boundary and preserves debt/remainder. Local submit rejection, poisoned ingress, active-batch, acknowledgement, a fallible-system fault, and a declared required task/service fault enter Faulted before success. Optional task failure and failed asset reload with a last-good value remain observable without changing runtime lifecycle.
- **Verification:** App/gameplay/task and headless tracer-drive tests pass; source searches find no discarded gameplay lifecycle `Result`, synchronous wait in first-party admission/shutdown, source reopen from an attempt, reused mutable generation, unowned Starting/RetirementIncomplete state, enum-only pause behavior, false Ready/Running/Stopped publication, or speculative public runtime-factory trait; ADR status leaves Editor Play integration to U17 and Winit target integration, replay, system stepping, recovery, and universal Host abstractions deferred.
...
- **Dependencies:** U5's runnable headless recipe/start/runtime path and U15's disposable Windows/Linux feedback path.
...
- **Approach:** One concrete root `EditorProjectSession` owns the single Play owner: Empty, PreparingPlay(pending intent and exact revision), Starting(`RuntimeStartAttempt`), Active(`RuntimeInstance`), or RetirementIncomplete(`RuntimeStartAttempt`). `nara_tooling` owns only `EditorPlayCommand`, `EditorPlayView`, generation-stamped runtime edit/observation requests, Apply Changes models, and Pending/Applied/Rejected/Failed operation results; neither tooling nor egui imports a recipe, start attempt, runtime instance, process handle, native lease, or root-private refresh job. An accepted document patch increments the revision and invalidates the current recipe. Play/Restart against an absent or stale recipe records one pending intent, exposes PreparingPlay, and automatically continues after the Host prepares the exact current revision; validation/build failure produces one Failed result without a second click. One Editor frame drives the concrete owner and derives the tooling view from that single truth. Close Scene, external reload, Restart, second-document Play, and editor exit create one observable pending workspace intent, cancel PreparingPlay/Starting, or Stop Active. Successful retirement/Stop resumes the original intent; timeout cancels it, retains RetirementIncomplete or runtime CloseIncomplete ownership plus the document, and exposes Retry Retirement/Close. A second intent is Busy. Restart from Running, Paused, or Faulted first stops the owner, refreshes if needed, and begins a fresh attempt; incomplete retirement/close, Starting, Stepping, and Stopping reject competing Restart. This Interface remains valid whether Play is initially in-process or later a child-process Adapter.
- **Editor control contract:** The egui adapter renders `EditorPlayView` and submits commands rather than inventing lifecycle state. Empty exposes Play; PreparingPlay and Starting expose Cancel/Stop; Running exposes Pause, Stop, and Restart; Paused exposes Resume, Step Fixed Tick, Stop, and Restart; Stepping and Stopping expose status with conflicting controls disabled; Faulted exposes Dispose/Restart; RetirementIncomplete and CloseIncomplete expose Retry Retirement/Close. Request acceptance and eventual operation result remain separate. Pending workspace intents are visible, and every unavailable control remains dimensionally stable and disabled rather than disappearing.
...
- **Test scenarios:** Covers AE8 and AE11. A public inspector patch increments the document revision; low-level Start with the prior recipe rejects, while integrated Play enters PreparingPlay, prepares the current validated recipe, then automatically transitions through Starting to Running without a second click. Pause reaches Applied and one exact StepFixedTick consumes a generation-stamped runtime edit. Stop completes and same-revision Restart publishes fresh mutable state while retaining only edit-document truth. A selected runtime value persists only after explicit Apply Changes produces and applies a valid patch; unsupported/prefab-ambiguous/stale requests reject without document mutation. A second Start/Restart during PreparingPlay or Starting is Busy; Close/Reload/exit cancels Host preparation or the start attempt and resumes its single pending intent only after clean retirement. Preparation failure publishes no RuntimeInstance. Scheduled systems run during Play, and paused Editor frames advance only permitted real-time work. Start shutdown timeout retains RetirementIncomplete; runtime close timeout retains CloseIncomplete; each exposes a distinct retry command and can later finish without losing the document. Faulted keeps Dispose/Restart and cannot be mistaken for incomplete retirement/close. Click-level egui tests cover every visible status, enabled command, disabled conflict, pending intent, and Pending/Applied/Rejected/Failed result. Static boundaries reject tooling ownership of a bare `World`, RuntimeStartAttempt, RuntimeInstance, root/private snapshot/recipe/refresh types, process/native authority, direct gameplay mutation, and silent owner removal.
- **Verification:** Tooling, concrete Host integration, egui click/view, scene Play, and workspace runtime tests pass; the public patch -> automatic recipe preparation -> Starting -> Play -> Pause Applied -> StepFixedTick -> Stop -> Restart flow is executable without a private world path or second author action; searches find no `ScenePlaySession`-owned bare `World`, authority-bearing owner in tooling, unowned Preparing/Starting/RetirementIncomplete/CloseIncomplete state, silent owner drop, or editor-only schedule authority.
...
### U23. Jointly Admit or Reject Runtime Ownership Proposals

- **Goal:** Give ADR 0082 and ADR 0084 one independent, atomic decision gate after their headless, Editor, and desktop evidence trials, before any Publish decision can rely on them as current authority.
...
- **Dependencies:** U5's headless lifecycle evidence, U17's Editor command/view and stop-first evidence, U13's desktop driver/close evidence, and U19's ADR-governance validation.
...
- **Files:** ADRs 0082/0084, their reciprocal related/refinement metadata, affected Accepted ADRs only if both proposals are accepted or both rejected, `docs/architecture/adr/{README,implementation-status}.md`, `docs/architecture/nara-foundation.md`, `tests/architecture_docs.rs`, and one immutable review record under `docs/knowledge/engineering/`.
- **Approach:** An independent reviewer evaluates every admission metric and non-claim against exact U5/U17/U13 artifacts, fault matrices, static boundaries, and Host-lifetime evidence. The only completed outcomes are atomic **Accept Both** or **Reject Both**. Accept Both changes both decision statuses together, adds reciprocal refinement metadata, updates current-authority foundation/ledger text, and records remaining implementation gaps truthfully. Reject Both changes both statuses together, removes their proposed owner model from current-successor completion claims, and creates a successor decision/plan before release work continues. Incomplete or mixed evidence leaves both Proposed and blocks U20; one Accepted plus one Proposed/Rejected is invalid. This review does not add code, rename types from preference alone, or infer desktop/Editor parity from headless evidence.
- **Test scenarios:** Deleting one product evidence set, leaving one ADR Proposed, accepting only one ADR, citing either proposal as current authority before the atomic status change, omitting a failed success metric, or updating foundation text before acceptance fails governance checks. Complete matching evidence permits one atomic Accept Both result; a coherent architectural contradiction permits one atomic Reject Both result with explicit successor linkage.
- **Verification:** The independent review record names exact evidence revisions and a metric-by-metric verdict; ADR files, catalogue, ledger, related/refinement links, foundation authority, and active plan agree on one atomic outcome; `cargo nextest run --test architecture_docs` passes; U20 remains blocked unless the result is Accept Both.
...
- **Approach:** Derive the ADR file, catalogue, and implementation-ledger sets from repository contents and require exact equality without hard-coding the current count. Validate decision-status vocabulary independently from implementation status; do not add `Trial`. Parse optional `Atomic Admission Group` metadata, require every named member to exist, require reciprocal declarations of the same complete group, and require all members to share one decision status so a group cannot be partly Accepted, Proposed, or Rejected. Require reciprocal links for the active plan supersession relation and ADRs changed by this successor, reject duplicate/extra/stale catalogue entries, validate paths/symbol anchors on changed ledger rows, and assert that ADRs 0045 and 0081 expose the same canonical-v1 capability set. Every Accepted row that is not implemented must retain a non-empty remaining gap and admission trigger; Proposed decisions must not be cited by product or foundation documents as current authority. Preserve Proposed for bounded evidence trials and use the implementation ledger for partial/implemented proof. Unrelated historical prose/link cleanup remains trigger-driven backlog rather than a U20 blocker.
- **Test scenarios:** Removing any ADR file, catalogue entry, or ledger row; adding an extra/stale/duplicate entry; using an unknown decision or implementation status; naming an absent atomic-group member, declaring the group on only one member, declaring divergent member sets, changing only ADR 0082 or ADR 0084 to Accepted/Rejected, breaking the active-plan or changed-ADR reciprocal relation; naming a missing path or symbol anchor in a changed row; diverging the 0045/0081 canonical capability sets; omitting the remaining gap or admission trigger from an Accepted non-implemented row; or citing a Proposed ADR as current product authority fails a focused fixture. An unrelated legacy narrative link is outside this unit. Accepted with partial implementation remains valid when its residual work and trigger are explicit, Proposed with an evidence unit remains valid, and no test assumes a fixed ADR count.
- **Verification:** Architecture-doc tests prove exact dynamic set equality, status separation, atomic-admission-group membership/status integrity, active-plan/affected-link integrity, changed-row anchors, residual-evidence completeness, and Proposed-authority exclusion; the current 78/78 repository passes as observed evidence rather than a baked-in constant; no full historical link crawl becomes a release prerequisite.
...
- **Dependencies:** U7's immutable candidates, U8's ownership cleanup, U13's desktop first playable, U14's approved headless gate, U17's editor runtime integration, U19's governance validation, and U23's atomic **Accept Both** result for ADRs 0082/0084.
- **Entry trigger:** Headless evidence made the desktop funding decision and candidate packages exist, but the complete pre-publication diff, clean-to-desktop, supported window-slot configuration, desktop runtime tails, exact candidate delivery, and final publication/next-slice decisions remain unreviewed.
...
- U23 independently reviews exact U5/U17/U13 evidence and atomically accepts or rejects ADRs 0082/0084; only Accept Both aligns them as current authority and permits U20.
...
- U4 proves complete AE5 preflight composition plus type-directed/stable-slot configuration/disable/order, custom schedule registry parity, image-limit configuration, terminal shutdown naming, and candidate-owned acquire-then-fail shutdown in both build and finish; hidden nested composition cannot partially publish an `App`, ordinary authors avoid stable infrastructure vocabulary, and cross-plugin replacement remains trigger backlog.
...
- U5 proves a concrete Host-owned `RuntimeStartAttempt`, a headless-profile `RuntimeRecipe` over exact leased snapshot values, nonblocking pollable admission/shutdown, acquire-then-fail and Host-cancel retirement, truthful RetirementIncomplete ownership, native registry/scene preflight, exact complete StepFixedTick, qualified sticky runtime faults, and bounded shutdown without claiming synchronous-hook preemption, a public object-safe factory, or Editor/desktop coupling.
...
- U19 enforces dynamic exact equality among ADR files, catalogue entries, and implementation-ledger rows, mechanical atomic-admission-group membership/status equality, plus bounded active/affected relations while keeping decision status separate from implementation evidence and unrelated historical prose outside the release gate.
...
- U20 waits for U7/U8/U13/U14/U17/U19 plus U23 Accept Both, reviews the complete pre-publication successor, completes clean-to-desktop, slot-configuration, frame-time, memory, build, candidate, editor-workflow, public-coverage, and labeled docs-only clean-room measurements, reruns invalidated protocols with preserved lineage, and records Publish/Redirect/Stop against one exact final candidate plus the next-slice rule.
