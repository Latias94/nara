...
- AE24. Given a renamed-dependency external extension, it can register work in the documented public
  ...
  prove escalation into the sticky managed-runtime fault contract.
...
- One deterministic 2D arena-survival wave with a semantic non-text HUD, desktop input, headless parity, a precommitted headless productivity gate, separate Rust-author and non-program-author docs-only clean-room journeys, metrics, three-workspace CI, and standalone GitHub pre-release evidence artifacts built from the source-distributed reference workspace.
...
U18 -> U15 and U19 remain independent evidence/governance lanes; U15 must converge before U14. After U6, U17 may integrate the same U24 start path into Editor Play while U13 proceeds, and U8 may restore internal task-set ownership. A Continue result from U14 admits U7 candidate packaging. Once U13, U17, and that Continue result exist, U27 compares bounded project-creation candidates and freezes one reversible supported path for the non-program-author clean-room subject. Once U5, U24, U25, U13, U17, and U19 have supplied the complete evidence/governance set, U23 independently decides ADR 0084's thin runtime and ADR 0082's outer Host, then verifies the accepted combination. U20 may proceed only when both required authorities are Accepted, or an Accepted successor replaces a rejection and affected product evidence has been rerun, and when U27 has supplied that project-creation subject. U20 then joins U7/U8/U27 evidence, reviews the complete pre-publication successor, runs the independent Rust-author public-API and non-program-author project-creation journeys, and measures the exact final candidate. Either journey's failure prevents Publish. Only a pinned canonical Publish approval record admits U21's reviewed protected pre-release publisher.
...
- **Test scenarios:** A renamed root dependency defines its own system/set, registers it in
  ...
  not claim managed-runtime escalation; U5/U6 separately prove that sticky fault contract.
...
- **Verification:** `nara_fs`, root, and independent generated-project tests plus a fresh docs-only
  ...
  broaden support. Public documentation limits the supported project-creation candidate to those two
...
- **Entry trigger:** Headless and desktop product evidence admitted candidate packaging and U27 supplied the non-program-author project-creation subject, but the complete pre-publication diff, Editor workflow, independent Rust-author and non-program-author clean-room journeys, exact candidate delivery, artifact size/startup, and final publication/next-slice decisions remain unreviewed.
...
- **Non-program-author project-creation clean-room journey:** A different fresh runner begins outside an existing project and uses U27's documented candidate to select one product preset, create and open the project, build and Run or click Play, Stop, and reopen it. The contributor must not edit Rust or Cargo, configure the Rust plugin slot from the other journey, or manually reconcile Cargo features, `nara.toml` capabilities, and plugin groups. Record creation/run/reopen success, generated product-closure coherence, task time, and every undocumented intervention as independent admission evidence for OQ-037. Failure blocks a claim that Nara already provides a mature integrated project-creation workflow, but may redirect publication to an explicitly Rust-author-only milestone; success in the Rust-author journey cannot satisfy this flow, and one successful path does not accept the permanent project-creation Interface. Asset rename/move, prefab identity, and broad Editor persistence remain outside this tracer and keep their ADR 0083/0087/0091 evidence gates.
...
- **Architecture handoff:** Before U21, update the architecture index, foundation summary, runtime-composition harness status, and implementation ledger against the reviewed release commit. Record actual owners/symbols, Accepted ADRs, unresolved OQs including OQ-015/OQ-031/OQ-033/OQ-039 through OQ-043, every design draft that still needs rebaseline, and any legacy ADR template debt that still needs a substantive owner review; this handoff does not activate any deferred plan or permit empty boilerplate sections.
...
- U27 freezes one reversible project-creation candidate plus alternatives/reversal evidence before U20 and supplies only the non-program-author subject. U20 then runs simplification and bug/regression-focused review over the complete pre-publication diff, resolves P0/P1 findings, repeats affected gates, removes abandoned attempts, completes separately identified Rust-author public-API and non-program-author project-creation journeys in disposable credential-free runners/VMs, and pins the canonical approval record plus exact final run/artifact identities before U21. Either journey's failure prevents Publish; candidate code never executes during read-only evidence ingestion, and that job holds no repository or Release mutation authority.
...
- U27 compares versioned template/root action, CLI, and Editor New Project candidates before implementation, lands only the smallest reversible path that generates one coherent Cargo/manifest/plugin/startup/product closure through a live parent capability, private same-filesystem sibling stage, and atomic no-replace publication under destination/parent/link/cross-volume race fixtures, and supplies the non-program-author docs-only outside-project subject without accepting OQ-037 or arbitrary project trust.
...
- U20 waits for U7/U8/U13/U14/U17/U19/U27 plus U23-compatible Accepted authority, reviews the complete pre-publication successor, completes independent Rust-author public-API and non-program-author project-creation journeys plus clean-to-desktop, slot-configuration, frame-time, memory, build, candidate, editor-workflow, and public-coverage measurements in credential-free disposable runners, accepts collector data only through identity-bound budgeted redacted envelopes in a non-executing read-only ingestion job with no mutation authority, reuses U14 evidence only within U22's per-metric environment-equivalence classes, reruns all source/environment-invalidated protocols with preserved lineage, and pins one canonical Publish/Redirect/Stop record by approval commit, Git blob object ID, and SHA-256 bytes against the exact final candidate plus next-slice rule.
