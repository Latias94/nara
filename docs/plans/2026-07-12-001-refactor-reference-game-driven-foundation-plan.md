...
- **Approach:** Measure the actual public workflow, not a synthetic harness: one data edit/reload, one structural Rust edit/rebuild/fresh runtime, clean checkout-to-headless success, clean checkout-to-manually-playable desktop success, public production coverage, frame-time P99, process memory, and cold/incremental build time. Record environment, mechanism, sample count, failures, and non-claims in one compact document. Use isolated temporary workspaces and a cross-platform helper where automation adds value; do not require canonical envelopes, Git ancestry policy, transitive source manifests, or release-grade provenance for local evidence. U20 may rerun or expand these measures against exact candidates.
- **Test scenarios:** The helper refuses the active dirty checkout as a measurement subject, preserves failed samples, separates incompatible environments, and leaves the source worktree unchanged. A valid run records the exact public entry point, mechanism, environment, count, and raw numeric samples. Private engine hooks or undocumented setup make public coverage fail. Missing desktop or headless success produces a named bottleneck, not a fabricated Continue verdict.
