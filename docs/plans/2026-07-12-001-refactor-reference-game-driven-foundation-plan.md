...
- The remaining critical path spans four distinct ownership contracts plus one counterfactual freeze: a pure plugin plan is the shared prerequisite; the thin code-first runtime owner, immutable startup-content snapshot, and persistent-apply eligibility guard may then proceed in parallel; U26 freezes the manual snapshot/App baseline only after content and target-World composition eligibility exist but before Host implementation; and the concrete product Host transaction combines the production owners.
...
- R36. After U4/U12/U29 converge but before U24 implementation, U26 must freeze and independently review an evidence-only manual raw-App tracer, caller-glue/concept inventory, and baseline record for the exact plan, snapshot, eligible target-World apply, fixed-tick task, and shutdown target. U25 later runs that unchanged tracer and the U24 Host tracer on the same current source revision, toolchain, content, command input, and U22 environment class while the manual path bypasses U24 ownership orchestration. Post-U26 changes to the tracer or its dependency surface are allowed only when the protocol proves them mechanically semantics-preserving; any change to concepts, caller glue, states, fault boundaries, or shutdown flow invalidates the comparison and requires a newly blinded baseline review before U24 continues. Landed U2/U3 samples remain historical lineage. The new model may continue only when it closes named failure/ownership gaps without exceeding U22's independently calibrated complexity limits; otherwise it must be simplified, redirected, or rejected before U6/U17/U13.
- R37. Every U14/U25/U20 result crossing from an untrusted or credential-free collector into trusted review, repository, or publication state must use one versioned non-executable evidence envelope with encoded-byte, depth, record-count, field-count, and string-length budgets. The envelope allowlists typed fields and safe relative identifiers, binds source run, source revision, protocol digest, environment class, subject, and payload digest, and carries normalized redacted records rather than arbitrary logs or shell text. Trusted parsing fetches by expected identity and size, uses structured APIs without shell interpolation, rejects links, special files, traversal, aliases, unexpected archive entries, forged identity/environment fields, and digest mismatch, and never executes candidate-controlled content. That ingestion remains read-only and secret-free with no repository, Release, OIDC, or publication authority; a separate reviewed step may commit only its immutable normalized digest-bound output. Raw logs that cannot be safely normalized remain in access-controlled, time-bounded artifact storage and are referenced only by non-secret identity and digest. A U20 Publish decision is itself a canonical bounded record pinned for U21 by approval commit, Git blob object ID, and SHA-256 bytes; candidate provenance alone never substitutes for that approval identity.
- R38. Before U20 begins its non-program-author journey, one bounded first-party Rust-hosted project-creation candidate must start outside an existing project, accept one author-facing product preset, and generate a mutually consistent Cargo feature ceiling, `nara.toml` capability request, first-party plugin composition, and runnable product entry without manual Cargo edits. The candidate is selected by a pre-implementation alternatives comparison, remains reversible, and supplies evidence to OQ-037 rather than accepting the permanent CLI/Editor/template Interface. It proves only a generated Cargo-backed Rust Host workflow; it does not require a future Adapter-authored or C#-only project to contain a Cargo graph, Rust entry point, or project-built native Host. This non-program-author journey and R33's Rust-author public-API clean room are independent evidence subjects; success in either cannot substitute for the other.
...
- AE23. Given a live authorized parent outside an existing project, an absent child destination, and one supported Rust-hosted product preset, the provisional first-party creation path builds in a private same-filesystem sibling stage and atomically publishes no-replace a project whose Cargo features, `nara.toml` request, plugin plan, startup content, and Run/Play action agree. Existing/non-empty or concurrently created destinations, parent identity drift, path escape, link/reparse injection, cross-filesystem publication, partial-write injection, unsupported presets, or generated closure drift reject without overwrite or a half-project. A fresh contributor can run, stop, and reopen the result without constructing infrastructure IDs or editing Cargo; success does not accept OQ-037's permanent Interface or prove a C#-only/prebuilt-Host project shape.
...
- Headless and desktop comparison subjects, independently sourced ranges, hard stops, sample rules, per-metric environment-equivalence classes, evidence-envelope rules, digest-bound source/environment invalidation rules, and decision logic are committed before U4/U12/U5/U24. U26 freezes the manual counterfactual after U4/U12/U29 converge and before U24; later units may bind public entry points but cannot rewrite the protocol or baseline semantics. U25 must Continue before the model spreads; a failed headless hard stop prevents U13; otherwise the final U14 decision requires both the authoritative headless wave and a manually playable desktop path.
...
- A versioned evidence review records the pre-implementation protocol, raw lineage, independent Rust-author public-API and non-program-author Rust-hosted project-creation clean-room journeys, the canonical approval-record identity, and the rule for selecting the next vertical slice before any successor implementation starts.
- One pre-U20 Rust-hosted project-creation candidate proves that a single product choice can generate a coherent Cargo-backed runnable project without exposing capability-closure or plugin-planning machinery; the permanent template/CLI/Editor and C#-only/prebuilt-Host shapes remain evidence-gated by OQ-037 and OQ-007 respectively.
...
- One deterministic 2D arena-survival wave with a semantic non-text HUD, desktop input, headless parity, a precommitted headless productivity gate, separate Rust-author and non-program-author Rust-hosted docs-only clean-room journeys, metrics, three-workspace CI, and standalone GitHub pre-release evidence artifacts built from the source-distributed reference workspace.
- One provisional first-party Rust-hosted project-creation path for the U20 clean-room trial, selected from bounded alternatives and explicitly not frozen as the permanent project-authoring Interface or evidence for a C#-only/prebuilt-Host project.
...
| The manual ownership baseline is designed after seeing U24 and biases Continue | Commit and independently review U26 after U4/U12/U29 converge but before U24; freeze its task, concept/glue inventory, owner graph, and evidence identities, and invalidate target-aware or semantic post-freeze edits. |
...
U9 through U3, U11, and U10 are landed baseline evidence: active-plan authority, Rust authoring, truthful capabilities, bounded manifest input, safe surface retirement, and bounded PNG ingest. The next serial priority is U22: independently calibrate and freeze the semantic evidence, envelope, and source/environment invalidation contracts before more of the measured path exists. U4 then replaces the overlapping plugin lifecycle and resolves the common profile/provider/schema input without depending on file content. From U4, U12's World-independent content snapshot, U29's persistent-apply eligibility guard, U5's code-first candidate/runtime lifecycle, and U28's public semantic-anchor proof proceed in parallel. U12 and U29 converge at U26, which performs the first manual target-World materialization and freezes the task-equivalent raw-App counterfactual without seeing U24. U24 is then the only product join: it combines U12, U4, U5, and U29's transitive eligibility proof through one explicitly registered obligation ledger while leaving the U26 baseline semantically unchanged. U25 compares those paths on the same current source and environment class; a Redirect or Stop blocks diffusion. U28 joins only before U6. Thus the ownership/content path is `U22 -> U4 -> (((U12 || U29) -> U26) || U5) -> U24 -> U25`, while `U4 -> U28` proceeds in parallel; U25 and U28 both gate U6. A hard stop exposed by U6 blocks desktop work immediately; otherwise U13 projects the same runtime driver path into the manually playable desktop game, and U14 makes the first product Continue/Redirect/Stop decision from both paths.

U18 -> U15 and U19 remain independent evidence/governance lanes; U15 must converge before U14. After U6, U17 may integrate the same U24 start path into Editor Play while U13 proceeds, and U8 may restore internal task-set ownership. A Continue result from U14 admits U7 candidate packaging. Once U13, U17, and that Continue result exist, U27 compares bounded Rust-hosted project-creation candidates and freezes one reversible supported path for the non-program-author clean-room subject. Once U5, U24, U25, U13, U17, and U19 have supplied the complete evidence/governance set, U23 independently decides ADR 0084's thin runtime and ADR 0082's outer Host, then verifies the accepted combination. U20 may proceed only when both required authorities are Accepted, or an Accepted successor replaces a rejection and affected product evidence has been rerun, and when U27 has supplied that Rust-hosted project-creation subject. U20 then joins U7/U8/U27 evidence, reviews the complete pre-publication successor, runs the independent Rust-author public-API and non-program-author Rust-hosted project-creation journeys, and measures the exact final candidate. Either journey's failure prevents Publish. Only a pinned canonical Publish approval record admits U21's reviewed protected pre-release publisher.
...
| M3 Independent content/composition/runtime contracts | U22, U4, U29, U12, U5, U26 | Calibrated subjects and evidence envelope predate implementation; U4 resolves the common provider/schema input; U12 records only World-independent document/schema truth; U29 proves static binding and per-apply target-World eligibility without extending that fingerprint; a sealed code-first App enters a truthful candidate/runtime lifecycle; and the manual counterfactual is independently frozen after U12/U29 converge but before Host work | Rules follow results, U12 captures runtime/native state or claims to certify live World topology, U29 lacks a pre-mutation recheck, U4 retains live plugin state or permits hook-time runner mutation, U5 guesses arbitrary resource ownership, aggregate residency escapes its budget, the baseline reviewer sees U24, evidence crosses trust without a bounded envelope, or a wrapper duplicates schedule authority |
...
| M5 Headless first wave | U22, U28, U6 | The small evidence contract predates implementation, a renamed-dependency extension proves the schedule/set anchors consumed by the game, then one authoritative wave reaches both terminal states through the public semantic-command/runtime path and produces a stable snapshot | Rules change after U6 starts, anchor ordering depends on a system function/absent/cross-schedule target, a hard stop is ignored, more than two broad legacy units are required, or any private engine hook is needed |
...
| M11 Rust-hosted project creation candidate | U27 | A pre-implementation comparison selects one reversible first-party Rust-hosted path, and an outside-project fixture generates one coherent Cargo-backed runnable product through live-parent, same-filesystem sibling staging and atomic no-replace publication without manual Cargo or infrastructure vocabulary | No candidate exists, generated capability layers diverge, a destination race can overwrite or escape authority, publication is partial, or the trial silently freezes OQ-037's permanent Interface or a universal Cargo project shape |
| M12 Final evidence | U20 | Pre-publication review, immutable lineage, independent Rust-author public-API and non-program-author Rust-hosted project-creation journeys, and exact-final-candidate measurements produce a commit/blob/SHA-pinned Publish/Redirect/Stop approval record plus one next-slice rule | Required U23 authorities are not Accepted/compatibly replaced, U27 is missing/stale, affected samples are stale, a frozen range is changed, P0/P1 findings remain, either journey needs hidden context or fails, or approval/artifact identity, digest, or retention is incomplete |
...
- If any U4-U5, U10-U12, U22, U24-U25, or U28-U29 unit leaves the runnable tracer with no new completed production, safety, or falsifiable evidence workflow, defer its unexercised subcontracts; if two consecutive non-safety units add no such workflow, stop infrastructure work and rewrite the next slice.
...
| U26 | Pre-Host manual ownership counterfactual | frozen raw-App tracer, inventory, baseline policy, evidence envelope | U4, U12, U22, U29 |
...
| U6 | Complete authoritative headless wave | `reference-game/`, public contract tests | U2, U10, U22, U25 Continue, U28 |
...
| U27 | First-party Rust-hosted project-creation candidate | project creation tests, candidate review, one reversible Rust-hosted product preset path | U3, U4, U13, U14 Continue, U17, U24 |
...
- **Evidence-envelope contract:** The protocol also freezes one canonical non-executable envelope schema and ingestion policy for U26/U25, U14, and U20. It budgets encoded bytes, depth, nodes, container items, records, per-record and aggregate fields, and single/total string bytes; allowlists typed fields and safe relative identifiers; and defines deterministic UTF-8 canonical encoding, ordering, duplicate rejection, and payload-digest coverage. A trusted caller supplies the expected run/source/protocol/subject/environment identity independently of collector bytes and verifies the outer transfer size/digest before the envelope can route or publish anything. Collectors emit only preclassified typed fields: sensitive and secret classifications are value-free markers, while arbitrary logs, free text, commands, links, special files, and unvalidated archives are forbidden rather than heuristically redacted after ingestion. Restricted raw logs remain access-controlled artifacts with explicit retention. Collector output is untrusted data even when the collector itself had no credentials.
...
- **Dependencies:** U4's resolved plan and closed App commit, U12's immutable startup-content snapshot, U29's persistent-apply eligibility guard, and U22's frozen protocol/evidence envelope. It does not depend on U5 or U24, and U24 may not begin until this unit is committed and independently reviewed.
...
- **Dependencies:** U4's pure `RuntimePlan`/private preparation, U5's sealed-App candidate/runtime lifecycle, U12's immutable startup-content snapshot, U22's frozen protocol, and U26's pre-Host manual baseline with U29 eligibility. U24 may add its own path but cannot semantically rewrite that baseline or its dependency surface.
...
- **Dependencies:** U24's minimal product start path, U26's frozen manual counterfactual, and U22's calibrated protocol/evidence envelope. U4/U12/U29 are transitive through U26; it precedes U6, U17, and U13.
...
- **Dependencies:** U2 authoring, U10 bounded assets, U28's public semantic schedule/set anchor proof, a Continue result from U25 over U24's complete product start transaction, and U22's precommitted evidence contract. U4/U5/U12 are transitive through U24; no window, surface, or desktop unit is a headless prerequisite.
...
- **Evidence transfer:** Collectors write only U22's bounded canonical envelope into the trusted review path. The trusted step fetches by independently expected identity, size, and digest; structurally parses and verifies source/protocol/subject/environment/payload identities; accepts only the protocol's preclassified typed fields; and rejects unknown, misclassified, or value-bearing sensitive/secret fields instead of guessing how to redact them. Archives must pass the same exact no-link/no-special-file/no-traversal table policy as release artifacts. Commands and diagnostic text are data, never interpolated. Raw process logs remain access-controlled artifacts when they cannot enter the typed envelope.
...
### U27. Trial a First-Party Rust-Hosted Project Creation and Product Preset Path

- **Goal:** Produce one reversible first-party Rust-hosted project-creation candidate that U20 can test from outside an existing project without pretending that OQ-037's permanent template/CLI/Editor Interface or a universal Cargo project shape is already known.
...
- **Files:** `.github/workflows/ci.yml`, `tests/ci_policy.rs`,
  `crates/nara_fs/src/{capability,error,proof,replace}.rs`,
  `crates/nara_fs/src/platform/{windows,unix}.rs`,
  `crates/nara_fs/tests/{filesystem_contract,path_contract}.rs`, new
  `tests/project_creation.rs`, new `reference-game/tests/project_creation.rs`, new
  `docs/benchmarks/project-creation-candidate-review.md`, public user documentation, ADR 0070, the
  implementation ledger, and the one root/template/Editor candidate file set named by the
  pre-implementation comparison before code begins.
- **Approach:** Compare at least a versioned starter template invoked by a concrete root action, a small CLI command, and an Editor New Project adapter as bounded candidates. Record ordinary-author concepts, closure ownership, destination safety, failure atomicity, automation, and reversal cost before selecting the smallest candidate that can satisfy AE23; implement one candidate, not all three. One author-facing Rust-hosted preset owns generation of the compiled Cargo feature ceiling, `nara.toml` request, first-party plugin composition, startup content, and product entry so users cannot create a split-brain closure. Creation receives a live host-issued parent-directory capability and one validated absent child name, creates an unpredictable private sibling staging directory under that same parent and filesystem, writes and validates the generated project through opened capability-relative handles and the normal U3/U4/U24 path, then publishes with a platform-proven atomic no-replace directory operation bound to the same parent identity. Existing targets, parent identity drift, cross-filesystem publication, unsupported no-replace guarantees, or any race fail closed without overwriting or accepting a partial destination. Generated code may use private/generated glue, but the project author does not construct IDs, plans, candidate typestates, or Host internals. This unit creates no package manager, native-code trust grant, universal project builder, or permanent UI promise; it does not prove a C#-only project, prebuilt Host, managed build/export, no-Cargo project, or second author language. Arbitrary downloaded/external projects remain governed by OQ-031.
- **Test scenarios:** Covers AE23. From a randomized cwd/home outside any project, the selected candidate creates its supported preset, and machine checks prove Cargo features, `nara.toml`, plugin closure, startup content, and the Run/Play path agree. Exact path/name/size/count limits succeed; limit+1, invalid names, traversal, links/reparse points, existing/non-empty destinations, unsupported presets, generated closure drift, write/validation failure, and interrupted publication leave no accepted partial project. Race fixtures create the final destination after preflight, swap or replace the parent path, inject a junction/reparse point or symlink, and force cross-volume or unsupported no-replace publication; each rejects without overwriting either project and without escaping the original live parent capability. The generated project builds, starts, stops, and reopens through public product/Editor paths without a manual Cargo edit or stable-infrastructure import. A counterfactual comparison records why the other candidate shapes were not selected and which evidence would reverse the choice.
- **Verification:** `nara_fs`, root, and independent generated-project tests plus a fresh docs-only
  task pass. `.github/workflows/ci.yml` owns required
  `project-creation-windows-ntfs` and `project-creation-linux-ext4` jobs. Each job probes and records
  its live OS/kernel, architecture, local filesystem, volume/mount identity, and capability receipt,
  then proves collision, parent-identity, link/reparse, and cross-volume/mount cases; a probe mismatch
  fails rather than skips. Injected platform-seam tests cover other native result branches but do not
  broaden support. Public documentation limits the supported Rust-hosted project-creation candidate to those two
  live-proven environments, and every other platform/filesystem returns `Unsupported` or `Unproven`
  without a name-precheck/rename fallback. The candidate review records alternatives,
  metrics, risks, selected source revision, and reversal criteria before implementation.
  Public-surface searches find no author-visible capability-closure duplication, composition IDs,
  plan/candidate/Host choreography, ambient destination write, first-party allowlist, or claim that
  one successful trial accepts OQ-037.
...
- **Dependencies:** U7's immutable candidates, U8's ownership cleanup, U13's desktop evidence, U14's approved first-playable gate, U17's editor runtime integration, U19's governance validation, U23's compatible Accepted decisions or Accepted successors for every required ADR 0082/0084 product authority, and U27's frozen Rust-hosted project-creation candidate.
- **Entry trigger:** Headless and desktop product evidence admitted candidate packaging and U27 supplied the non-program-author Rust-hosted project-creation subject, but the complete pre-publication diff, Editor workflow, independent Rust-author and non-program-author clean-room journeys, exact candidate delivery, artifact size/startup, and final publication/next-slice decisions remain unreviewed.
- **Files:** new `.github/workflows/reference-game-evidence-ingest.yml`, new
  ...
  `tests/{measurement_policy,ci_policy,evidence_envelope,project_creation}.rs`, new
  `docs/benchmarks/{reference-game-baseline,reference-game-evidence-review}.md`,
  `docs/benchmarks/project-creation-candidate-review.md`, the normalized-artifact schema at
...
- **Approach:** First run simplification and bug/regression-focused review over the complete pre-publication diff, resolve P0/P1 findings, remove abandoned attempts, and repeat affected gates. Then run the Rust-author and non-program-author Rust-hosted clean-room journeys below as independent subjects with separate runners, permissions, entry states, allowed edits, completion records, and failure results; neither may substitute for the other or imply an unimplemented no-code configuration path. Any API/docs fix discovered by either journey returns to full pre-publication review and repeats both affected journeys. Only after the review and both required journeys pass without further source changes does U20 rerun U7's read-only pipeline from the exact reviewed product source commit to create the final publishable candidate; all earlier and clean-room candidates remain lineage only.
...
- **Non-program-author Rust-hosted generated-project clean-room journey:** A different fresh runner begins outside an existing project and uses U27's documented candidate to select one Rust-hosted product preset, create and open the project, build and Run or click Play, Stop, and reopen it. The contributor must not edit Rust or Cargo, configure the Rust plugin slot from the other journey, or manually reconcile Cargo features, `nara.toml` capabilities, and plugin groups. Record creation/run/reopen success, generated product-closure coherence, task time, and every undocumented intervention as independent admission evidence for OQ-037. Failure blocks a claim that Nara already provides a mature integrated Rust-hosted project-creation workflow, but may redirect publication to an explicitly Rust-author-only milestone; success in the Rust-author journey cannot satisfy this flow, and one successful path does not accept the permanent project-creation Interface. This subject does not prove C#-only gameplay, a prebuilt Host, managed build/export, a no-Cargo project, or a second author language. Asset rename/move, prefab identity, and broad Editor persistence remain outside this tracer and keep their ADR 0083/0087/0091 evidence gates.
- **Clean-room isolation:** Human and agent-assisted runs use an isolated HOME, trust-separated caches, sanitized child-process environment, no secrets/OIDC, no SSH/Git/Cargo/release credentials, and no persistent checkout credentials. The two journeys use independent evidence envelopes and cannot reuse a publishable artifact identity.
- **Final candidate and approval:** Before accepting final bytes, verify repository owner/name, workflow path and reviewed definition SHA, allowlisted event/trigger chain, protected source ref, run ID/attempt, product source commit, immutable artifact IDs, sizes, retention policy/deadline, and digests. Preserve U14's raw lineage and apply U22's digest-bound source/environment rules per metric: reuse is permitted only when every required environment field remains in its predeclared equivalence class; an out-of-class, ambiguous, shared engine/build/config, or unknown change reruns the full affected suite. Add final candidate size/startup and Editor workflow results; do not re-collect already valid U14 metrics merely to change their ranges. One canonical bounded approval record contains the Publish/Redirect/Stop decision, next-slice rule, reviewed product source commit, protocol digest, journey identities, and exact final candidate identities/digests. After that record is committed and reviewed, U20 completion evidence records its protected-default-branch approval commit, Git blob object ID, and independent SHA-256 byte digest; changing any field creates a new U20 approval version rather than mutating the published authority.
...
# RGF-U27
cargo nextest run --locked -p nara_fs --test filesystem_contract --test path_contract --test-threads=1
...
- U27 freezes one reversible Rust-hosted project-creation candidate plus alternatives/reversal evidence before U20 and supplies only the non-program-author Rust-hosted subject. U20 then runs simplification and bug/regression-focused review over the complete pre-publication diff, resolves P0/P1 findings, repeats affected gates, removes abandoned attempts, completes separately identified Rust-author public-API and non-program-author Rust-hosted project-creation journeys in disposable credential-free runners/VMs, and pins the canonical approval record plus exact final run/artifact identities before U21. Either journey's failure prevents Publish; candidate code never executes during read-only evidence ingestion, and that job holds no repository or Release mutation authority.
...
- U17 replaces the tooling bare Play `World` with a concrete Editor Host-owned recipe/start/runtime owner and UI-neutral tooling commands/views, proving `NoActiveDocument`, PreparingPlay Cancel without an attempt, Starting Cancel through owned RetiringPlay, and the public patch -> one-click recipe preparation -> Play -> Pause Applied -> StepFixedTick -> Stop -> fresh Restart flow, explicit safe-point runtime edit versus Apply Changes semantics, complete egui state/control coverage, and one pending workspace intent without freezing an in-process topology or creating a root/tooling dependency cycle.
...
- U27 compares versioned template/root action, CLI, and Editor New Project candidates before implementation, lands only the smallest reversible Rust-hosted path that generates one coherent Cargo/manifest/plugin/startup/product closure through a live parent capability, private same-filesystem sibling stage, and atomic no-replace publication under destination/parent/link/cross-volume race fixtures, and supplies the non-program-author docs-only outside-project subject without accepting OQ-037, arbitrary project trust, or a universal Cargo/C#-only project shape.
