...
> **Execution-authority correction (2026-07-31):** This plan is superseded and grants no execution
> authority. Current execution scope comes from the user or host control plane. The active successor
> only orders technical dependencies and verification gates.

## Goal Capsule

- **Objective:** Finish a first-playable, independently consumable reference-game slice before adding more evidence infrastructure, proving Nara's Rust authoring, modular composition, authorized content boot, runtime lifecycle, headless/desktop parity, renderer safety, honest editor persistence, and delivery path.
...
- **Stop conditions:** Revise the active unit when its reference-game acceptance test disproves the proposed boundary. Stop and re-plan if two consecutive non-safety engine units add no playable behavior or completed production or safety workflow, if proof machinery grows larger than the behavior it protects, or if the first playable requires a broad return to the legacy 33-unit waterfall.
...
- The remaining critical path is product-shaped: retain the committed manual snapshot/App tracer as a small pre-Host reference, compose the concrete product Host over the landed plan/content/runtime/apply owners, immediately build the complete headless wave, then close the desktop and editor/package workflows. The Host proves equivalence with the manual tracer inside its own acceptance tests; no separate evidence subsystem blocks playable work.
- Two narrow correctness gates now protect that path without adding product layers: U29 guards the
  first target-World persistent apply without serializing World-independent content loading, while
  U28 makes public schedule/set compatibility a prerequisite only of the expanded headless game.
...
- R22. After the headless and desktop paths are playable, U14 must record a lightweight reproducible baseline for data and structural Rust edit-to-result latency, clean checkout-to-headless and desktop success, public production coverage, frame-time P99, memory, and build time. Broader sample populations, external collector ingestion, candidate size/startup, and release-grade provenance belong to U20 after U7 produces immutable candidates.
...
- R32. The landed U22 protocol remains a measurement vocabulary and source-lineage record, not a prerequisite framework that can block U24 or U6. U24 owns the early architecture reversal check through focused tests against the committed manual tracer; U6 blocks U13 only on a concrete headless correctness or safety failure, and U14 records the first product decision only after both playable paths exist.
...
- R36. After U4/U12/U29 converge, U26 must retain one committed manual raw-App tracer plus a concise human-readable inventory of caller concepts, ownership transfers, fault boundaries, and shutdown outcomes before U24 implementation. U24 must run the manual and product paths over the same committed success task and a three-case failure matrix: failure before owner acquisition, the existing late persistent-hook rejection after App ownership, and incomplete retirement after an obligation transfer. The comparison records diagnostic class, publication visibility, retained owner/state, and cleanup outcome; the Host may strengthen ownership but may not hide or weaken a manual failure. The ordinary product action exposes only project authority/selection, profile or run intent, semantic control input, product outcome, and structured diagnostics as concept categories; candidate, admission, publication, retirement, and any new category stay private unless this plan is revised before U6. Git history preserves the pre-Host baseline; no AST policy, transitive source-closure manifest, blinded-review protocol, or byte-identical rebinding system is required.
- R37. Untrusted external collector or release-candidate results crossing into trusted review or publication state must use the bounded non-executable evidence envelope and identity checks defined by U22. Local U24/U14 regression and measurement results may use ordinary committed test output or compact structured records; they must not grow a trust-ingestion subsystem before an external boundary exists. U20 still owns release-grade normalized ingestion and the canonical approval record pinned for U21.
- R39. Before the independent reference game expands its use of public scheduling anchors, only explicitly documented
  semantic `ScheduleLabel` and `SystemSet` anchors may form compatibility dependencies: schedule
  labels for typed registration, joinable sets for membership and before/after ordering. Each anchor must
  state its owning schedule, producer-visible inputs, consumer-visible completion, deferred flush,
  skip/run-condition, fault/error, and cleanup semantics. Concrete first-party system functions,
  private sets, incidental registration order, absent targets, and cross-schedule targets must not
  be presented as ordering guarantees. The first-playable inventory is exactly
  `CoreStage::FixedUpdate`, `FixedUpdateSet::Simulate`, `GameplayCommandSet::Consume`, and
  `GameplayCommandSet::Capture`, each with an explicit participation mode. `App::seal` must build
  and validate their owning schedule, set graph, and deferred-command policy before configuration
  becomes immutable.
- R40. After prefab expansion, explicit stable-ID component records must remain the complete
  persistent composition. Canonical-v1 provider validation rejects required-component and intrinsic
  `ComponentHooks` metadata that can participate in persistent insertion/removal. Every persistent
  target-World apply then revalidates actual component metadata, including World-registered
  `ComponentHooks`, plus relevant lifecycle observers at a flushed, exclusive, mutation-free safe
  point; active construction/removal hooks or observers reject before allocation. Runtime-only ECS
  and post-publication World-local hooks/observers remain valid runtime behavior, but a later
  persistent apply must repeat the check and rejects while a matching hook remains installed. A
  future authoring preset or catalog-derived closure
  requires evidence through OQ-043 and a separately Accepted ADR or successor.
- R41. In the first known-schema editor workflow, Saved advances only after a concrete persistence Host atomically publishes canonical bytes and returns a receipt bound to document identity, captured source revision, canonical digest, previous and published file identity, achieved file data/metadata synchronization, and verified candidate publication. The session holds the available cooperative file lock, binds Save to the last observed disk identity/digest, and rechecks with `ExpectedTarget::Identity`; a detected external change, deletion, or lost lock rejects without publication. Pre-publication failure leaves the document dirty and preserves prior bytes; an ambiguous post-publication result enters a typed persistence-uncertain state and never claims rollback or Saved. Dirty Close and Exit ask Save, Discard, or Cancel before runtime retirement; Cancel preserves the current runtime, while Save/Discard commits a pending close only after the selected persistence action is resolved. Reopen must reproduce the saved content through the retained project authority. The first slice explicitly claims neither strong CAS against non-cooperating writers nor parent-directory power-loss durability, unavailable-schema degraded authoring, recovery journals, multi-file transactions, or a general concurrent-writer policy.
...
- AE21. Given U26's committed pre-Host manual tracer and the same committed plan, content fixture, command input, fixed-tick task, and bounded stop target, the manual raw-App path and U24 ordinary product action produce the same authoritative result. On the shared pre-owner failure, late persistent-hook rejection, and incomplete-retirement cases, both paths preserve the same diagnostic class and publication result while the Host makes any retained owner more explicit rather than weaker. The ordinary product surface contains only project authority/selection, profile or run intent, semantic control input, product outcome, and structured diagnostics; candidate, admission, publication, retirement, a false terminal state, an unowned lifecycle edge, or an unapproved additional concept category blocks U6.
...
- AE24. Given a renamed-dependency external extension, it can register work in the documented public
  schedule anchor, join/order against the documented set anchors, and observe their completion,
  deferred flush, skipped, faulted, and cleanup
  behavior without naming a first-party system function. Ordering against an absent anchor or the
  same anchor name in another schedule is rejected or proven ineffective explicitly rather than
  silently claimed as a valid edge. This unit proves App/domain error semantics; U5/U6 separately
  prove escalation into the sticky Nara-owned `RuntimeInstance` fault contract.
- AE25. Given a persistent component with Bevy required-component or intrinsic `ComponentHooks`
  metadata, static provider validation rejects it. Given a late registration or an active lifecycle
  observer in the target `World`, Scene/Prefab/Inspector/migration/direct persistent apply flushes
  deferred work, establishes a post-flush canary baseline, and holds the World exclusively. A
  fresh-target path checks hooks plus event-global/component-global scopes before allocation and
  keeps exclusivity through insertion; an existing/reserved-target path checks all four observer
  scopes before persistent mutation. Rejection changes no document identity, applicable post-flush
  World/resource/foreign-entity canary, or deferred work. Post-publication runtime observers
  remain allowed, but every later persistent apply repeats the check. A successful path materializes
  exactly the explicit post-expansion persistent set and claims no arbitrary hook rollback.
- AE26. Given a known-schema scene edit, an in-memory command cannot mark it saved. A matching Host write receipt advances the saved checkpoint; a failed, stale, forged, or mismatched receipt leaves it dirty and preserves the prior file. Dirty Close or Exit accepts only explicit Save, Discard, or Cancel, and reopening after Save reproduces the persisted component value before Play starts from that revision.
...
- The playable slice has reproducible data and structural Rust edit-to-result baselines plus clean-to-headless and clean-to-desktop success records; no broader latency or usability claim is made before U20's larger evidence run.
- U26 preserves the committed pre-Host manual task in Git and a concise ownership/fault inventory. U24 proves equivalent authoritative behavior while replacing ordinary caller choreography with one product action, and that focused acceptance check gates U6. Concrete headless correctness or safety failures block U13; U14 evaluates the already-playable headless and desktop paths without becoming a benchmark framework.
...
- A renamed-dependency extension proves the four first-playable schedule/set anchors, their
  participation modes, seal-time graph/deferred-policy validation, and App/domain lifecycle
  semantics without relying on first-party systems or incidental registration order.
- Static provider and per-apply target-World fixtures prove that canonical-v1 persistent
  composition is exactly the explicit post-prefab set while runtime-only and post-publication
  observer behavior remains available outside a persistent apply.
- U20 records the release-grade evidence review, an independent Rust-author public-API clean-room journey, canonical approval-record identity, and next-slice rule only after the product, editor, and standalone candidate workflows exist.
...
- A known-schema single-document editor loop with receipt-backed Save, dirty-close confirmation, reopen, and Play/Stop/Restart; unavailable-schema lossless authoring, recovery journals, and concurrent-writer policy remain separate trials.
...
- One deterministic 2D arena-survival wave with a semantic non-text HUD, desktop input, headless parity, lightweight post-playable metrics, three-workspace CI, standalone candidates, and a later independent Rust-author docs-only clean-room journey.
- Focused repairs to `legacy U6`, `legacy U7`, `legacy U10-U14`, `legacy U28`, or `legacy U31` only when a named first-playable test fails without them.

**Deferred until evidence triggers a later vertical slice**

- OQ-007's first-party optional C#/.NET gameplay Adapter Trial, any other second author language, a universal behavior host, dynamic non-Rust ECS components, or a stable native dynamic ABI. Read-only precedent work and the later evidence-gated disposable feasibility research described by OQ-007 do not authorize production APIs in this plan.
...
- A first-party New Project/template/CLI generator and a non-program-author generated-project claim. OQ-037 remains open until a real external team or a measured new-project creation failure proves that cloning or copying a versioned starter is insufficient; evidence/release machinery cannot manufacture that product requirement.
...
- Full inherited visibility and hierarchy semantics not required by the flat first-wave scene, nested prefab rebase, edit-while-playing merge, unavailable-schema lossless editing, recovery journals, durable multi-file transactions, and multi-instance editing. Parented spatial/visibility content that requires those semantics fails closed in this slice.
...
- Post-RGF governance reduction: shrink `AGENTS.md` back to workflow and durable prohibitions,
  keep `nara-foundation.md` as an ownership map rather than an implementation ledger, move embedded
  ADR follow-up questions into the open-question catalogue, and replace this large execution
  artifact with a short charter plus current-unit brief and trigger backlog. This cleanup follows
  the product, editor, and package evidence; it does not reopen proven crate boundaries or block a
  playable workflow to redesign documentation machinery.
...
13. **Measure product workflows after they exist.** U22 preserves useful metric names, privacy classes, and release-boundary envelope rules, but it does not authorize a pre-playable benchmark framework. U24 uses ordinary focused tests for architecture reversal, U6/U13 fail on concrete correctness or safety defects, and U14 records a small reproducible baseline only after both playable paths exist. U20 alone owns external collector trust, larger samples, and release-grade Continue/Redirect/Stop evidence.
...
15. **Keep file authority in a concrete product host without freezing a universal host API.** U4 derives provider selection and a frozen schema/codec validation input from U2 declarations plus U3's side-effect-free settings/profile candidate. This proves the current native Rust provider/binding slice, not the admission order for every future Schema source. U12 extends the existing root project host with the first file-backed snapshot load and process-lifetime capabilities, borrows that validation authority, and stores only runtime-independent Catalog semantics, its fingerprint, and common lineage in content, never Rust registration functions, provider IDs, or native bindings. U24's root Host owns private profile recipes and runtime start attempts. `nara_project`, `nara_reflect`, `nara_asset`, and `nara_scene` keep parsing, validation, import, and spawn semantics in their domains. OQ-007 owns whether a future Adapter module generation supplies Catalog input before planning or joins as another immutable Host input. Extract another public crate only if two real hosts require the same boundary.
...
18. **Use headless as a fail-fast checkpoint, not the product verdict.** U6 must close concrete simulation, lifecycle, content, and safety failures before U13 begins, but Nara is a game engine product only when the same public path produces a manually playable desktop game. U14 records a lightweight first baseline from both paths. U20 later adds Editor, clean-room, and exact release-candidate evidence; U21 publishes only an approved candidate.
...
27. **Keep the counterfactual small and put reversal inside the product Host acceptance gate.** (session-settled: user-approved — chosen over a standalone Git/AST evidence system and separate evidence gate: the proof machinery had grown larger than the gameplay it was meant to protect.) U26 keeps the already committed manual raw-App tracer plus one concise ownership/fault record. U24 runs that task and the ordinary Host action over the same fixture, proves authoritative equivalence, and rejects unexplained author concepts or weaker ownership; Git history supplies the pre-Host comparison point.
28. **Publish semantic schedule anchors, not implementation order.** U28 freezes the first-playable
    compatibility inventory at exactly `CoreStage::FixedUpdate`, `FixedUpdateSet::Simulate`,
    `GameplayCommandSet::Consume`, and `GameplayCommandSet::Capture`. Each entry declares whether it
    is a schedule label or joinable phase plus its lifecycle semantics. `App::seal` builds and
    validates the owning schedule, set graph, and deferred policy; external code may depend on those
    anchors but never on first-party system functions, private subsets, incidental registration
    order, or an absent/cross-schedule target. This does not add a scheduler wrapper, global stage
    DSL, `PluginPlan` schedule metadata, or public access to every internal set.
29. **Keep persistent component composition explicit in canonical version 1.** U29 closes the gap
    between explicit stable-ID documents and Bevy insertion behavior without making a content
    snapshot certify a future `World`. Provider validation/freeze rejects static required-component
    and intrinsic `ComponentHooks` participation; every persistent apply then flushes deferred work
    and rechecks the real target-World component and lifecycle-observer topology under an exclusive
    borrow immediately before mutation. Prefab-expanded component records remain authoring truth;
    runtime-only components and post-publication observers retain normal Bevy ECS freedom, but a
    later persistent apply must repeat the check. U12 and U29 therefore proceed in parallel and
    converge before U26/U24 first materializes content. Ergonomic presets and catalog-derived
    closures remain OQ-043 alternatives and do not enter the first-playable API.
30. **Make editor persistence truthful before broadening authoring admission.** U17's known-schema path may report Saved only after a concrete Host write returns a receipt bound to the document, revision, and digest; dirty close must confirm or cancel, and reopen must recover the persisted bytes. This is a bounded product trial under Proposed ADR 0091, not acceptance of recovery journals, multi-file atomicity, or concurrent writers. Proposed ADR 0090 lossless unavailable-schema authoring remains deferred until the first known-schema editor loop is complete.
...
#### Product-First Ownership Checkpoint

```mermaid
flowchart LR
    Baseline[U26 committed manual raw-App task] --> HostGate[U24 Host acceptance tests]
    Host[U24 ordinary product action] --> HostGate
    HostGate -->|same result and simpler caller| Headless[U6 complete headless wave]
    HostGate -->|unowned state or leaked choreography| Simplify[Simplify U24 before gameplay]
    Headless --> Desktop[U13 desktop first playable]
    Desktop --> Measure[U14 lightweight product baseline]
```

The pre-Host task remains inspectable in Git, but Nara does not build a second policy engine to prove
that history exists. U24 owns the focused reversal test: the Host must preserve the authoritative
result, strengthen failure and retirement ownership, and remove candidate/admission choreography from
the ordinary caller. Once that passes, the complete game supplies the next evidence instead of another
...
- Public scheduling compatibility is limited to `CoreStage::FixedUpdate`,
  `FixedUpdateSet::Simulate`, `GameplayCommandSet::Consume`, and
  `GameplayCommandSet::Capture`; seal-time validation freezes their supported graph and deferred
  policy, while later units must extend the inventory before using another public edge.
- Persistent binding eligibility gains static provider certification plus an exclusive
  target-World pre-apply check. `ProjectContentSnapshot` remains World-independent and carries only
  explicit document/schema truth; U26/U24 and every direct persistent-spawn path own the World-local
  check.
...
| The Host comparison drifts away from the pre-Host task | Keep the committed U26 tracer and compact owner/fault record, then require U24 to execute the same semantic fixture and authoritative assertion; review semantic changes directly in Git instead of maintaining a second dependency-closure policy. |
...
U9 through U5, U10-U12, U22, U28, and U29 are landed foundation evidence. U26 now closes only the smallest pre-Host reference: the committed manual raw-App tracer plus a concise ownership, fault, and shutdown record. U24 is the sole remaining product join and includes focused reversal coverage against that committed task. U6 follows immediately and completes the authoritative headless game; concrete correctness or safety failures are repaired before U13 projects the same simulation into the desktop first playable. The playable critical path is `U26 -> U24 -> U6 -> U13`; U14 is the immediate post-playable evidence unit.

U18 -> U15 and U19 remain independent feedback/governance lanes. After U13, U17 closes the known-schema editor save/reopen/Play loop while U7 builds a checkout-independent candidate; neither waits for a release-grade provenance system. U8 may restore internal task-set ownership in parallel when it does not interrupt the product path. Once U13, U17, U19, and U24 have supplied product evidence, U23 independently decides ADR 0084's thin runtime and ADR 0082's outer Host, then verifies the accepted combination. U20 joins U7/U8/U14/U17 evidence, runs the independent Rust-author public-API journey, and measures the exact final candidate. Only its pinned Publish approval admits U21's reviewed protected pre-release publisher. New Project productization remains an OQ-037 follow-up triggered by external adoption evidence, not a release prerequisite.
...
| M3 Independent content/composition/runtime contracts | U4, U29, U12, U5, U26 | U4 resolves the common provider/schema input; U12 records World-independent content truth; U29 proves per-apply eligibility; U5 owns the sealed code-first runtime; and U26 retains a runnable pre-Host manual task with a concise owner/fault record | U12 captures runtime/native state, U29 lacks a pre-mutation recheck, U4 retains live plugin state, U5 guesses arbitrary resource ownership, aggregate residency escapes its budget, or U26 grows another policy framework |
| M4 Concrete product start and reversal gate | U24 | The root Host joins matching values, moves one sealed App plus registered ledger into an unpublished candidate, publishes only after startup, matches U26's authoritative result, and hides advanced lifecycle choreography from the ordinary caller | A start attempt reopens source, loses/double-owns an obligation, blocks the Host, creates a promoted-but-unpublished owner, accepts mismatched lineage, adds unexplained caller concepts, or requires a universal Host/factory Interface |
| M5 Headless first wave | U28, U6 | The renamed-dependency extension proves the schedule/set anchors consumed by the game, then one authoritative wave reaches both terminal states through the public semantic-command/runtime path and produces a stable snapshot | Anchor ordering depends on a system function/absent/cross-schedule target, a correctness or safety failure is ignored, more than two broad legacy units are required, or any private engine hook is needed |
...
| M7 First product evidence | U14 | The complete headless and desktop paths produce a small reproducible edit/build/runtime baseline and an explicit next-bottleneck decision | Measurement grows into a pre-product framework, depends on the active checkout, or overstates the sample population |
| M8 Editor and internal ownership | U17, U8 | The concrete Editor Host proves receipt-backed known-schema Save, dirty-close/reopen, Start/Pause/Step/Restart, while asset/task characterization preserves same-frame and next-frame behavior after ownership moves | Tooling reports Saved without a write receipt, drops a dirty document, regains a bare `World` or live runtime owner, runs a stale recipe, or changes observed game behavior |
...
| M11 Final evidence | U20 | Pre-publication review, immutable lineage, an independent Rust-author public-API journey, and exact-final-candidate measurements produce a commit/blob/SHA-pinned Publish/Redirect/Stop approval record plus one next-slice rule | Required U23 authorities are not Accepted/compatibly replaced, affected samples are stale, a frozen range is changed, P0/P1 findings remain, the journey needs hidden context or fails, or approval/artifact identity, digest, or retention is incomplete |
| M12 Public release | U21 | The final permission-bearing workflow passes independent review; no-checkout write jobs publish exact U20 artifacts around a read-only draft smoke, then anonymous public smoke passes before announcement | Publisher review, tag/version/branch authority, artifact identity/retention, archive preflight, immutable state, or either download smoke fails |
...
- If U6 exposes a concrete headless correctness or safety failure, do not begin U13. Repair the smallest owning contract and rerun the complete wave.
- If U14 cannot reproduce both headless and manually playable desktop workflows without private hooks, record the bottleneck and redirect the next product slice instead of expanding measurement machinery.
- If U24 shows that the candidate/runtime/Host split adds unexplained ordinary-author glue, an unowned state, or weaker fault ownership than the committed manual task, do not begin U6. Simplify or reject the failing ADR premise while its reversal cost is still local.
...
- If any remaining non-safety unit adds no playable behavior or completed production workflow, defer its unexercised subcontracts. If proof or policy code becomes larger than the behavior it protects, stop it and return to U24/U6/U13/U17/U7 product work.
...
- `docs/architecture/open-questions.md` OQ-007 records C#/.NET as the leading optional gameplay-language hypothesis, the evidence ladder for bounded feasibility research and a separate Trial, and its non-commitments.
...
- `docs/knowledge/engineering/subagents/2026-07/2026-07-16-bevy-godot-early-architecture-research.md`
  records the source-bound schedule-anchor, persistent-component composition, continuous-edit,
  package-removal, Editor session, settings-scope, and schema-metadata pressure behind R39/R40 and
  the deferred OQs.
- `docs/knowledge/engineering/subagents/2026-07/2026-07-16T164357Z-bevy-lifecycle-observer-and-deferred-schedule-verification-027c48803a9442d8930a3d0f558bafd3.md`
  records the precise five-event/four-scope observer model, dynamic-hook escape, deferred-policy
  bypasses, and package-removal co-ownership evidence used by U28/U29 and PX-38.
- `repo-ref/bevy/crates/bevy_app/src/main_schedule.rs`,
  `repo-ref/bevy/crates/bevy_ecs/src/{schedule/config.rs,lifecycle.rs,observer/centralized_storage.rs}`,
  and `repo-ref/bevy/crates/bevy_ecs/src/component/{mod.rs,required.rs}` provide the mature typed
  schedule/required-component mechanisms whose compatibility and persistence semantics Nara keeps
  explicit rather than inheriting wholesale.
...
| U28 | Public semantic schedule/set anchor compatibility | anchor lifecycle docs, external renamed-dependency ordering fixture | U4 |
| U29 | Explicit persistent component composition and hook boundary | persistent-binding eligibility, scene exact-composition and hostile-hook fixtures | U4 |
...
| U26 | Minimal pre-Host manual ownership reference | committed raw-App tracer and concise owner/fault record | U4, U12, U29 |
| U24 | Concrete root Host start transaction and reversal check | root Host, start attempt, candidate transfer, manual/Host equivalence tests | U4, U5, U12, U26, U29 |
| U6 | Complete authoritative headless wave | `reference-game/`, public contract tests | U2, U10, U24, U28 |
| U13 | Single-window desktop input/render wave | reference-game desktop, sprite/UI/wgpu regressions | U6, U11, U24 |
| U14 | Lightweight first-playable product baseline | small reproducible headless/desktop measurements and bottleneck record | U6, U13, U15 |
| U17 | Honest editor Save, reopen, and Play integration | tooling, concrete persistence Host, egui adapter, editor workflow tests | U13, U24 |
| U23 | Independent runtime/Host decision and compatibility review | ADR 0082/0084 evidence, authority docs, governance tests | U5, U13, U17, U19, U24, U26 |
...
U9, U1-U5, U10-U12, U18, U22, U28, and U29 are retained below as landed baseline evidence and verification contracts, not pending implementation. Their historical triggers explain why the work was admitted; execution derives completion from commits and verification records and begins the remaining critical path by closing the now-minimal U26 record before U24.
...
### U22. Record the First-Playable Measurement Vocabulary

- **Goal:** Preserve the landed metric vocabulary, privacy classifications, invalidation rules, and bounded external evidence envelope without making them a prerequisite benchmark framework for product implementation.
- **Requirements:** R3, R22, R32, and R37; AE22.
- **Dependencies:** U3's truthful project/capability baseline plus the completed U10/U11 safety boundaries.
...
- **Files:** new `tests/{measurement_policy,evidence_envelope}.rs`, new `docs/benchmarks/reference-game-first-playable-protocol.md`, bounded calibration/envelope fixtures in `docs/benchmarks/data/`, ADRs 0048/0049/0068 as evidence-boundary refinements, `docs/architecture/adr/implementation-status.md`, `docs/architecture/nara-foundation.md`, and focused verification records under `docs/knowledge/engineering/`.
- **Approach:** The landed protocol names semantic headless/desktop workflows, edit classes, public coverage, runtime/build measurements, environment fields, privacy classes, and source invalidation. U14 may reuse those names for compact local records but is not required to satisfy release-grade sample or provenance machinery. U20 owns larger populations, candidate measurements, clean-room evidence, and any final Continue/Redirect/Stop decision.
- **Evidence-envelope contract:** The canonical non-executable envelope remains mandatory only when untrusted external collector or candidate bytes cross into trusted review or publication. It budgets shape and strings, binds expected identities and digests, forbids executable/free-form payloads, and preserves restricted raw logs outside the repository. U24 and local U14 tests do not cross that boundary and use normal test or compact measurement records.
...
- **Ancestry scenario:** Release-grade evidence records name their protocol version and source revision. Local product tests rely on committed fixtures and Git history rather than a custom ancestor policy.
- **Evidence-envelope scenarios:** Covers AE22. Every encoded-byte/shape/domain limit succeeds exactly at the limit and rejects at limit+1 before typed decode or trusted publication. Absolute/traversing/aliased paths, symlinks, special files, unexpected archive entries, missing/substituted/duplicate expected-identity fields, payload or outer-transfer digest mismatch, shell metacharacters, and path/username/proxy-URL/credential canaries reject with static privacy-safe codes. Valid envelopes byte-round-trip canonically without executing or interpolating payload text, and restricted raw-log references disclose no host path or secret.
- **Verification:** The landed policy and envelope fixtures remain green; U14 can emit a compact local baseline without the external-ingestion path, and U20 can reuse the bounded envelope for release-grade evidence. No generic benchmark runner becomes a prerequisite for U24, U6, U13, U17, or U7.
...
- **Dependencies:** U4's resolved profile/provider/schema-validation input, U10's bounded image importer, and U22's frozen semantic evidence contract. U3 is transitive through U4; U12 is World-independent and proceeds in parallel with U29, while `App` construction and runtime lifecycle remain U4/U24/U5 work.
...
### U28. Prove Public Semantic Schedule/Set Anchor Compatibility

- **Goal:** Give external systems a small stable ordering surface with explicit lifecycle semantics
  before the reference game expands beyond its current `FixedUpdateSet` dependency.
- **Requirements:** R3 and R39; AE24.
- **Dependencies:** U4's closed App composition and public external-consumer path. It does not depend
  on project content, runtime Host ownership, Editor, or desktop rendering.
- **Entry trigger:** The independent reference game already schedules gameplay in
  `FixedUpdateSet::Simulate`, and first-party domains order through `GameplayCommandSet`, `InputSet`,
  task, diagnostic, extraction, and render sets. Rust visibility currently does not distinguish a
  supported semantic anchor from an implementation detail or define what deferred, skipped,
  faulted, absent, and cross-schedule edges mean.
- **Files:** ADR 0003 and only narrowly related owner docs, `crates/nara_app/src/lib.rs`,
  `crates/nara_gameplay/src/lib.rs` plus their focused in-crate contract tests, new
  `crates/nara_app/tests/schedule_compatibility.rs`, new
  `tests/fixtures/schedule-extension/renamed-root/`, new
  `tests/schedule_extension_contract.rs`, `reference-game/tests/public_surface.rs`,
  `docs/architecture/nara-foundation.md`, the implementation ledger, and migration documentation.
- **Approach:** Implement ADR 0003's complete first-playable inventory, not a new discovery pass:
  `CoreStage::FixedUpdate` is a public schedule-label participation point, while
  `FixedUpdateSet::Simulate`, `GameplayCommandSet::Consume`, and
  `GameplayCommandSet::Capture` are public joinable phases in that schedule. Document for each its
  entry inputs, consumer-visible completion point, deferred-command flush, run/skip behavior,
  App/domain fault or error behavior, and cleanup/retention relation. Before `App::seal` succeeds,
  build and validate the owning schedule and required set graph, require automatic deferred
  insertion, and reassert final deferred application before configuration closes. A relation using
  `before_ignore_deferred`, `after_ignore_deferred`, or equivalent chaining explicitly opts out and
  cannot pass the public-anchor conformance claim; U28 does not add a scheduler wrapper solely to
  detect that trusted advanced escape. A public configuration that otherwise invalidates an anchor
  contract rejects sealing, and no public schedule-configuration mutation remains after sealing.
  This proves no total order among otherwise unordered phase peers; consumers must declare their own
  necessary relation, and general ambiguity detection remains outside U28's compatibility claim. Domain-private ordering stays private;
  concrete product composition owns validated cross-domain edges. Keep Bevy's typed scheduling
  substrate and Nara's existing `configure_sets` shape. Do not add a scheduler wrapper, global stage
  DSL, `PluginPlan` schedule metadata, dynamic string registry, or public exports for every
  first-party system/set.
- **Test scenarios:** A renamed root dependency defines its own system/set, registers it in
  `CoreStage::FixedUpdate`, and joins/orders only against the three documented set anchors.
  Per-anchor fixtures observe
  the promised entry/completion state, deferred visibility, allowed skip behavior, App/domain fault
  outcome, and cleanup/retention timing. A hostile configuration that disables or changes required
  automatic deferred insertion, moves an anchor, or invalidates the required set graph rejects at
  seal; seal reasserts final deferred application and the sealed App exposes no later configuration
  escape. A fixture using an ignore-deferred edge may seal as trusted advanced code but fails the
  public compatibility oracle and observes no promised visibility. Unordered peer systems are not
  reported as having a stable relative order merely because the anchor graph builds. An absent target, private target, target in a
  different schedule, and direct reference to a first-party system function cannot be presented as
  a supported edge. Permuting unrelated registration does not change the semantic result. U28 does
  not claim Nara-owned `RuntimeInstance` escalation; U5/U6 separately prove that sticky fault contract.
- **Verification:** Focused App/domain schedule tests, the renamed-dependency fixture, and the
  reference-game public-surface test pass. Rustdoc and ADR review identify every supported anchor
  and no more; dependency/search review finds no external first-party system-function ordering,
  ambient main-loop insertion, new scheduler abstraction, or false cross-schedule guarantee.

### U29. Enforce Explicit Persistent Component Composition And Hook Boundaries

- **Goal:** Prove that persistent spawn composition remains exactly the explicit post-prefab
  document set and that every native binding/apply is eligible without extending a World-independent
  catalog/content fingerprint to cover live Bevy topology.
- **Requirements:** R3, R5-R10, and R40; AE25.
- **Dependencies:** U4's selected provider set and frozen schema-validation input; U1/U2 are
  transitive through U4. It proceeds in parallel with U12, U5, and U28, then joins U12 before U26's
  first target-World materialization and therefore before U24.
- **Entry trigger:** Nara's `Component` derive permits Bevy `#[require]` and lifecycle hooks, while
  scene codecs insert components one by one and the catalog fingerprint records neither required
  closures nor hooks. Adding a requirement can therefore change runtime composition without a
  schema/fingerprint change, and insertion order can change hook side effects that scene despawn
  cannot roll back.
- **Files:** `crates/nara_ecs/src/lib.rs`, `crates/nara_ecs_derive/src/lib.rs`, `crates/nara_reflect/src/{authoring,registry,codec,schema}.rs`,
  `crates/nara_reflect_derive/src/lib.rs`, `crates/nara_reflect/tests/` including derive UI fixtures,
  `crates/nara_scene/src/{document,spawn,tests}.rs`, new
  `tests/scene_component_composition.rs`, `reference-game/tests/authoring.rs`, ADRs 0006/0081,
  OQ-043, `docs/architecture/nara-foundation.md`, the implementation ledger, and migration docs.
- **Approach:** Keep canonical-v1 documents explicit: after prefab expansion, every durable
  persistent component is present by stable ID. Eligibility has two phases. First, provider
  validation/freeze registers the native component in an isolated scratch ECS registry when needed
  and rejects required-component or intrinsic `ComponentHooks` metadata that can participate in
  persistent insertion/removal. Second, every Scene, Prefab, Inspector, migration, or direct
  persistent apply flushes deferred work, captures its rejection baseline, takes one exclusive
  target-`World` borrow, and rechecks the real `ComponentInfo` values including World-registered
  dynamic hooks. A fresh-target path checks every matching event-global/component-global
  `Add`/`Insert`/`Discard`/`Remove`/`Despawn` observer before allocation and retains the exclusive
  borrow through target allocation and persistent insertion, preventing a new entity-scoped
  registration after the target exists. An apply to a target that already exists or was reserved
  before the guard additionally checks matching entity and entity+component observers before its
  first persistent mutation. Late metadata or an active matching lifecycle observer rejects the
  applicable path. Post-publication World-local hooks and runtime observers remain allowed; a later persistent
  apply rejects while a matching hook remains installed, and for a matching observer may instead
  wait for an explicit Host safe point that disables that observer.
  Runtime-only components retain normal Bevy behavior, and post-spawn systems may derive transient
  runtime state that export does not confuse with document truth. Failed scene application may
  remove newly owned entities but does not claim to reverse arbitrary external hook effects. Do not
  expose a generic public hook-introspection mirror: any needed Bevy-version-coupled hook-presence
  probe stays narrow and private to `nara_ecs`. Do not add a generic component-dependency trait,
  preset API, or catalog closure; OQ-043 keeps explicit
  authoring presets and a future stable-ID catalog closure as alternatives.
- **Test scenarios:** Derive-based and manual persistent bindings with a required component or
  intrinsic hook reject during provider validation/freeze with focused diagnostics; provider/codec
  escape paths cannot bypass the check. A fixture then freezes an otherwise eligible provider,
  installs late metadata, a dynamic `World::register_component_hooks*` hook, or an event-global/
  component-global lifecycle observer through direct or deferred World registration, and proves
  that a fresh-target apply flushes and rejects under exclusive revalidation before allocation. A
  separate already-existing/reserved-target fixture installs entity and entity+component observers
  and rejects before its first persistent mutation. Both fixtures capture their baseline after the mandatory flush;
  rejection leaves document revision, post-flush target `World`, stable-identity registry, resource
  canary, and foreign entity unchanged, adds no deferred work, and never runs the hook/observer. A
  post-publication World-local hook or runtime observer remains valid for normal runtime work; a
  later persistent apply rejects while the hook remains installed or waits for the admitted safe
  point to disable the observer. Runtime-only components still
  exercise Bevy requirements/hooks successfully. Scene, Prefab, Inspector-style add, migration,
  export, and direct persistent spawn agree on the explicit set; the World-independent fingerprint
  never claims to certify the live observer/component-registration topology.
- **Verification:** ECS/reflect/derive/scene/root/reference-game authoring gates pass; compile-fail
  and runtime fixtures cover static provider plus dynamic-hook and all observer-scope target-World
  escape attempts. Source review finds no inferred omitted persistent component, fingerprint claim
  over live World topology, missing exclusive pre-mutation recheck, public hook-introspection mirror,
  false hook rollback claim, runtime-only regression, or speculative preset/dependency API.
...
- **Review correction gate:** The source-bound review bound to `c9eb073` and scoped code-diff fingerprint `2de76cb46e24cef02e80c531c1a5d7039d001ea7` is recorded in `docs/knowledge/engineering/subagents/2026-07/2026-07-16-rgf-u5-runtime-code-review.md`. U5 remains incomplete until every P1 in that record is closed or an independently reviewed replacement proves the same invariant. In particular, an admission/start failure or abnormal Drop must not silently destroy or make unreachable an unfinished owner; fault capture must not be replaceable through candidate/driver World access; a ready candidate with a pre-existing fault must not publish false Running state; platform target retirement must remain possible before runtime close removes its mutation authority; CloseIncomplete helpers must actually retry within their budget; and real required task/service integration failures must exercise the production fault bridge. Panic containment is outside U5 and must be removed unless a separate Accepted decision and fault-state tracer define unwind, partial-mutation, native-callback, and cleanup consequences. Primary examples may retain the advanced candidate choreography during U5, but ordinary error propagation must not discard its retirement ownership; U24 owns the permanent concrete action and author-facing concept check.
...
- **Review regression scenarios:** Directly drop every ownership-bearing admission/start failure; exercise a Pending close owner through abnormal Drop and an observable process-owned quarantine or equivalent retained owner; replace/remove every runtime-installed fault resource through both candidate and driver scopes; report a fault after startup readiness but before promotion; enter Winit with Stop already pending; recover a CloseIncomplete task within the remaining helper budget; and produce required/optional task and service failures through real integration systems. Tests must prove no silent participant destruction, unreachable owner, false Running/Stopped state, lost first fault, provider-before-surface release, or ordinary error propagation that loses retirement ownership. U5 requires public runner-boundary and state-machine coverage for the pending-Stop ordering; U13 owns the supported native event-loop smoke.
- **Verification:** App/gameplay/task, code-first candidate/runtime, sealing, and runner-boundary tests pass; every P1 in the bound U5 review record has a named regression and closure result; source review finds no discarded engine-owned gameplay lifecycle result, direct platform `App::run_once`, arbitrary-resource obligation inference, unreachable unfinished owner, replaceable mandatory fault bridge, false Running/Stopped result, second schedule/time/tracker authority, project/content import in the runtime module, repeated once-only shutdown hook, unadmitted panic containment, ordinary error propagation that makes retirement ownership unreachable, or public universal runtime/factory trait. U24 owns product construction attempts, the permanent ordinary action, and fresh reconstruction; U17 owns Editor Play integration; U13 owns the native event-loop smoke.

### U26. Freeze the Manual Raw-App Ownership Counterfactual

- **Goal:** Preserve the already committed task-equivalent manual startup path as a small, inspectable pre-Host reference without building a second policy system.
- **Requirements:** R3 and R36; AE21's manual-path subset.
- **Dependencies:** U4's resolved plan and closed App commit, U12's immutable startup-content snapshot, and U29's persistent-apply eligibility guard. It does not depend on U5 or U24.
- **Persistent-apply baseline:** The manual tracer's first scene materialization must use U29's real
  path: flush deferred registration, take one exclusive target-World borrow, recheck actual component
  metadata and lifecycle observers, then allocate only on success. A late required/hook registration
  or observer fixture rejects against the post-flush baseline before allocation and becomes part of
  the frozen task/fault inventory; the counterfactual may not bypass the guard with direct codec or
  World insertion.
- **Entry trigger:** Historical U2/U3 evidence predates the authorized content/profile task, but the committed manual tracer now supplies the comparison point. Its attempted AST/Git dependency policy expanded beyond the behavior and triggered the plan's product-first stop condition.
- **Files:** `reference-game/tests/support/manual_raw_app_boot.rs`, `reference-game/tests/manual_raw_app_baseline.rs`, and new `docs/benchmarks/runtime-ownership-baseline.md`.
- **Approach:** Keep the landed tracer as executable source history. The compact record names the ordinary caller concepts, the `manual_caller` and `App::World` owner transfers, the fixed-tick task boundaries, clean and incomplete shutdown outcomes, and explicit non-claims. It cites the committed tracer revision instead of duplicating the call graph, hashing transitive source files, or attempting to prove future mechanical rebinding. Keep only three small counterfactual cuts in executable form for U24: a project-content failure before owner acquisition, the landed late persistent-hook rejection after App ownership, and one stalled required obligation that makes manual retirement incomplete after transfer. U24 may update lower-level APIs normally, but its acceptance test must still execute the same semantic fixture/cuts and compare their observable result; compatibility scaffolding added only for the baseline is forbidden.
- **Test scenarios:** Covers AE21's manual half. The tracer consumes the committed plan/snapshot and command input, materializes through U29's guard, reaches the declared first-tick snapshot, and demonstrates complete manual task retirement. The three failure cuts record diagnostic class, scene/runtime visibility, retained owner or process fallback, and cleanup outcome without claiming that a timeout always transfers ownership or that caller return proves worker retirement.
- **Verification:** The committed manual tracer test passes and the compact record matches its public concepts, task boundaries, owner transitions, faults, and shutdown assertions. There is no `runtime_ownership_baseline_policy` test, transitive dependency manifest, canonical ownership envelope, compatibility wrapper, candidate/runtime owner, or Host abstraction created solely for comparison.
...
- **Requirements:** R3, R12, R17, R26, R31-R32, and R36; AE8, AE13, AE17, and AE21.
- **Dependencies:** U4's pure `RuntimePlan`/private preparation, U5's sealed-App candidate/runtime lifecycle, U12's immutable startup-content snapshot, U26's committed manual task, and U29's persistent-apply eligibility guard.
- **Persistent-apply admission:** After plugin commit/registry freeze but before scene allocation, the
  fresh candidate World repeats U29's deferred flush plus exclusive component/observer recheck. A
  plugin-installed late required/dynamic-hook registration or lifecycle observer rejects before
  scene mutation/publication and retires through the existing attempt ledger. Post-publication
  World-local hooks/observers remain
  legal runtime behavior; every later persistent edit/spawn rejects while a matching hook remains
  installed and rejects or waits for the explicit Host safe point while a matching observer remains
  active.
...
- **Dependencies:** U2 authoring, U10 bounded assets, U24's complete product start transaction and integrated reversal check, and U28's public semantic schedule/set anchor proof. U4/U5/U12/U26/U29 are transitive through U24; no window, surface, desktop, or measurement unit is a headless prerequisite.
...
- **Test scenarios:** A scripted stream moves, instantiates repeated enemies from the committed prefab, fights, and reaches each terminal outcome at a stable tick; prefab round-trip/spawn preserves source identity, and same-tick death/completion resolves Defeated. Repeated runs from the same committed project and bundled or typed command task produce the declared snapshot. Zero/multiple fixed steps do not duplicate commands; local submit, ingress, Admit, Acknowledge, required-task, content, and tick-limit failures fault the run before success output; despawn leaves no stale stable identity; optional task and asset/data reload failure preserve last-good playable state. CLI tests cover bundled input, maximum ticks, JSON schema, stdout/stderr separation, exit codes, diagnostic privacy canaries, and shutdown. Source/public-surface checks reject an external command-path option or serialized scenario loader in this unit.
...
### U17. Complete Honest Editor Save, Reopen, and Play

- **Goal:** Close one known-schema editor workflow that can edit, save, close or cancel a dirty document, reopen persisted bytes, and run scheduled Play through a concrete Host-owned runtime lifecycle.
- **Requirements:** R3, R15, R17's editor-restart subset, R24, and R41; AE8, AE11, and AE26.
- **Dependencies:** U13's manually playable desktop path and U24's concrete start transaction over U5. It may proceed in parallel with U7 after the desktop workflow exists.
- **Entry trigger:** `ScenePlaySession` still owns a bare `World`; `MarkSaved` can advance an in-memory checkpoint without a successful write; dirty close removes the document; and tooling cannot request scheduled Play, Pause Applied, exact StepFixedTick, bounded Stop, or fresh Restart through one stable command/view Interface.
- **Files:** `crates/nara_tooling/src/{play,workspace,lib}.rs`, `crates/nara_tooling_egui/src/lib.rs`, `src/{project_host,lib}.rs`, `src/project_host/{runtime,persistence}.rs`, focused `nara_fs` persistence integration, `tests/{scene_play_mode,workspace_play_runtime,editor_persistence}.rs`, ADRs 0039/0047/0076 plus Proposed ADRs 0084/0091 as trial evidence, `docs/architecture/{runtime-composition-interface-design,nara-foundation,ui-product-boundaries-editor-dogfood-and-porting-strategy}.md`, and migration documentation.
- **Approach:** One concrete root `EditorProjectSession` owns the single Play owner and delegates every construction attempt to U24: Empty, PreparingPlay(pending intent and exact revision), Starting(`RuntimeStartAttempt`), RetiringPlay(`RuntimeStartAttempt`), Active(`RuntimeInstance`), or RetirementIncomplete(`RuntimeStartAttempt`). `RetiringPlay` is a status projection of the attempt's existing `Retiring` state, not another owner. `nara_tooling` owns only `EditorPlayCommand`, `EditorPlayView`, generation-stamped runtime edit/observation requests, Apply Changes models, and Pending/Applied/Rejected/Failed operation results; neither tooling nor egui imports a recipe, start attempt, runtime instance, process handle, native lease, or root-private refresh job. An accepted document patch increments the revision and invalidates the current snapshot/recipe. Play/Restart against an absent or stale recipe records one pending intent, exposes PreparingPlay, and automatically continues after the Host prepares the exact current revision; validation/build failure produces one Failed result without a second click. One Editor frame drives the concrete owner and derives the tooling view from that single truth. An explicit Cancel in PreparingPlay cancels the refresh plus pending Play intent, returns directly to Empty, and records Cancelled. An explicit Cancel in Starting retains the `RuntimeStartAttempt`, requests cancellation, and enters RetiringPlay; clean retirement returns to Empty and records Cancelled, while timeout/failure retains RetirementIncomplete ownership plus the document. Close Scene, external reload, second-document Play, and editor exit instead create one observable pending workspace intent, cancel preparation or request attempt retirement, and resume that retained workspace intent only after clean retirement. Stop applies only to a published Active runtime and follows Stopping -> Empty/CloseIncomplete. A second intent, including Restart during PreparingPlay or Starting, is Busy. Restart from Running, Paused, or Faulted first stops the owner, refreshes if needed, and begins a fresh attempt; RetiringPlay, incomplete retirement/close, Starting, Stepping, and Stopping reject competing Restart. This Interface remains valid whether Play is initially in-process or later a child-process Adapter.
- **Persistence receipt contract:** Delete `MarkSaved` as an author command. The root persistence Host snapshots canonical known-schema bytes plus document ID, source revision, digest, last observed disk digest, and `FileIdentity`; it synchronizes the temporary file and replaces only `ExpectedTarget::Identity` through the original project capability. A receipt advances the saved checkpoint only when it reports `AtomicNameSwitch`, achieved file data/metadata synchronization and name publication, the published identity equals the candidate identity, and identity evidence is `HandleBoundCandidate` or `PostPublishObserved`. A target change/deletion, lost cooperative lock, mismatched receipt, unsupported guarantee, or pre-publication error leaves the document dirty and the prior target intact. A result that may have failed after the name switch becomes `PersistenceUncertain`, keeps the document dirty, blocks blind retry, and requires explicit reopen/reconcile; it cannot claim prior-byte preservation. Windows handle-bound and Linux cooperative/name-bound matrices receive separate tests. Parent-directory synchronization is recorded when available but is not required for Saved, so this slice does not claim directory-entry survival across sudden power loss or strong CAS against a non-cooperating writer.
- **Persistence operation state:** `nara_tooling` owns UI-neutral persistence commands/views and retained `Saved`, `Opened`, `Rejected`, `Failed`, and `PersistenceUncertain` results; the root `EditorProjectSession` owns the capability, lock, byte snapshot, and asynchronous Host operation. One operation is `Idle`, `Saving(document, captured_revision)`, or `Opening(document)`; duplicates are Busy, and results remain until acknowledged. Save may snapshot a dirty document while Play is Active or OutOfDate because it does not mutate runtime state. Later edits do not retarget the in-flight write: an accepted receipt advances the saved checkpoint only to the captured revision, so the newer document remains dirty. Open/Reopen is allowed only with no live, starting, retiring, or incomplete runtime owner and no pending persistence/workspace operation; read, decode, or validation failure leaves the current workspace unchanged, while success activates the reopened document before recipe preparation.
- **Dirty-close ordering:** Close Scene and Exit present Save/Discard/Cancel before cancelling preparation or retiring a live runtime. Cancel clears the close intent and preserves the document plus current runtime exactly as they were. Save snapshots the current revision; only a matching Saved result commits the retained close intent and begins retirement. Save failure/conflict leaves the document and runtime open. Discard explicitly commits the close intent without changing disk, then begins retirement. RetirementIncomplete retains the document and committed close intent until retry succeeds; close never reports complete before both its selected persistence action and runtime retirement are terminal.
- **Persistence scope:** Reopen parses persisted bytes through the retained project capability and reproduces the saved revision/content before Play prepares a recipe. This unit does not accept ADR 0091's recovery journal, multi-file transaction, broad durability tier, or general concurrent-writer policy, and it does not claim ADR 0090 degraded authoring.
- **Editor control contract:** The egui adapter renders `EditorPlayView` and submits commands rather than inventing lifecycle state. Empty exposes a dimensionally stable Play control only when an active playable document exists; otherwise it is disabled and a submitted low-level request rejects with typed `NoActiveDocument`. PreparingPlay exposes Cancel only; Starting exposes Cancel only; RetiringPlay shows the retained pending intent with Play, Cancel, Stop, Restart, and conflicting workspace controls disabled. Running exposes Pause, Stop, and Restart; Paused exposes Resume, Step Fixed Tick, Stop, and Restart; Stepping and Stopping expose status with conflicting controls disabled; Faulted exposes Dispose/Restart; RetirementIncomplete and CloseIncomplete expose Retry Retirement/Close. Request acceptance and eventual operation result remain separate. Pending workspace intents are visible, and every unavailable control remains dimensionally stable and disabled rather than disappearing.
- **Persistence control contract:** The egui adapter renders the tooling persistence view rather than inferring disk state. Dirty exposes Save; Saving exposes progress with duplicate Save/Open and dirty-close completion disabled; a retained Failed/Rejected result exposes Retry after acknowledgement or correction; `PersistenceUncertain` exposes Reopen/Reconcile but never one-click retry. Save remains available while the Play view is Running, Paused, Faulted, or OutOfDate, and it captures the current document revision independently of runtime generation. Open/Reopen is disabled until runtime/preparation/retirement ownership is empty. Dirty Close/Exit presents a modal Save/Discard/Cancel decision before any Play state changes; after Save or Discard commits the close intent, conflicting controls remain disabled until persistence and retirement finish.
...
- **Deferred editor contracts:** This unit proves one-shot runtime edit/export and lifecycle control
  only. Continuous slider/gizmo/curve/text transactions remain the unimplemented ADR 0026 target;
  Editor-to-Play Live Edit, child-process transport, and general incremental observation/remote
  command sessions remain OQ-040, OQ-039, and OQ-041 respectively. U17 must not freeze their public
  carriers while satisfying the current in-process tracer.
- **Preparation and cancellation scenarios:** With no active document, Play stays disabled and a
  direct request returns `NoActiveDocument`. Cancel during PreparingPlay aborts refresh, clears the
  Play intent, returns Empty, and records Cancelled without constructing a start attempt. Cancel
  during Starting retains and retires the attempt through visible RetiringPlay, then returns Empty
  with Cancelled only after clean retirement; timeout preserves RetirementIncomplete ownership.
...
- **Persistence scenarios:** A known-schema edit is dirty until a real atomic replace returns every required identity, synchronization, publication, document, revision, and digest fact; forged, stale, mismatched, unsupported, or incomplete receipts cannot advance the checkpoint. Exact Windows/Linux receipt matrices pass, while weaker publication identity, missing file sync, non-atomic naming, parent/target mismatch, detected external write/deletion, and lost cooperative lock reject before publication and preserve prior bytes. A post-name-switch fault enters `PersistenceUncertain` without claiming rollback, Saved, or blind retry. An edit accepted during Saving leaves the document dirty after the captured revision becomes saved. Duplicate Save is Busy; retained Saved/Failed/Rejected/Uncertain results require acknowledgement. Dirty Close and Exit prompt before runtime retirement: Cancel preserves the runtime and document, Discard changes no disk bytes before retirement, Save begins retirement only after receipt acceptance, and Save failure leaves both open. RetirementIncomplete retains the committed close intent and document. Reopen from a random cwd/home through the original project capability reproduces the saved content; missing, unreadable, malformed, or invalid content leaves the current workspace unchanged and reports a retained failure. Click-level tests cover Save while Running/Paused/OutOfDate, Saving, Retry, acknowledgement, Open/Reopen availability, and every dirty-close outcome. Removing `MarkSaved` leaves no public command that can claim persistence without I/O. Unknown-schema input still follows current strict admission and is explicitly outside this unit.
...
- **Acceptance mapping:** The persistence scenarios above own AE26; the runtime and tooling scenarios below own AE8 and AE11.
- **Retirement-view scenarios:** Close/Reload/exit during PreparingPlay cancels Host preparation and resumes the retained workspace intent without inventing a retirement owner. During Starting it requests attempt retirement, enters visible RetiringPlay for every nonterminal retirement frame, keeps the pending workspace intent visible, and disables Play/Cancel/Stop/Restart plus conflicting workspace controls. Clean retirement returns to Empty or resumes the retained intent without flashing RetirementIncomplete; timeout or failure alone enters RetirementIncomplete. Faulted, RetiringPlay, RetirementIncomplete, and CloseIncomplete remain visually and behaviorally distinct in click-level tests.
- **Verification:** Tooling, concrete Host integration, persistence, egui click/view, scene Play, and workspace runtime tests pass; the edit -> Save receipt -> close -> reopen -> automatic recipe preparation -> Starting -> Play -> Pause Applied -> StepFixedTick -> Stop -> Restart flow is executable without a private world path or false saved checkpoint. Searches find no `MarkSaved`, dirty close without a decision, `ScenePlaySession`-owned bare `World`, authority-bearing owner in tooling, unowned Preparing/Starting/RetiringPlay/RetirementIncomplete/CloseIncomplete state, silent owner drop, or editor-only schedule authority.

### U13. Complete the Desktop Input and Render Wave

- **Goal:** Project the proven headless simulation into one correct, manually playable desktop product path without introducing a desktop-only authoritative gameplay path. Manual playability evidence may retune the shared committed scene data only when raw-App, headless, and desktop baselines update together and parity remains proven.
...
- **Dependencies:** U6's authoritative wave, U11's safe surface lifetime, and U24's product start transaction. U8 and U17 do not block desktop work; a concrete unresolved U6 correctness or safety failure does.
...
- **Files:** `reference-game/Cargo.toml`, `reference-game/src/{lib,input,resources,snapshot,systems,ui}.rs`, `reference-game/src/bin/{desktop,desktop_render_probe}.rs`, `reference-game/tests/{desktop_flow,desktop_parity,desktop_render,first_wave,headless_snapshot,plugin_composition,public_surface}.rs`, `crates/nara_identity/src/{domain,tests}.rs`, `crates/nara_winit/src/{lib,tests}.rs`, `crates/nara_input/src/lib.rs`, `crates/nara_gameplay/src/lib.rs`, `crates/nara_render/src/lib.rs`, `crates/nara_sprite_render/src/{types,extract,queue,tests}.rs`, `crates/nara_ui_render/src/{types,extract,queue,tests}.rs`, `crates/nara_render_wgpu/src/{backend,lib,sprite,ui}.rs`, `tests/{project_content_boot,project_host_boundary,runtime_driver_boundary,runtime_instance,plugin_composition}.rs`, ADRs 0039/0040/0041/0057/0078/0094, `docs/architecture/nara-foundation.md`, and migration documentation.
- **Approach:** Configure the existing desktop-window slot with an explicit `WindowPlugin` definition whose `PluginId` remains unchanged and whose settings must satisfy compiled/requested capabilities, dependencies, services, conflicts, and public title/size/event-loop expectations before installation. This proves same-plugin configuration, not replacement or arbitrary plugin equivalence. U24 authorizes the same committed U12 content closure for headless and desktop while retaining a separate lineage-bound snapshot and lease for each profile; equal content digests and source hashes prove content identity without mixing profile lineage or mutating the headless recipe. Replace Winit's raw-`App` runner path with a platform Host/driver that owns and drives `RuntimeInstance`, passes normalized events and elapsed time through its public drive boundary, and observes faults/close state without calling `App::run_once` directly. Extraction captures one owned, generation-stamped `RenderFramePacket` containing the admitted single window/target/view at the final pre-acquire boundary. Frame planning and the serialized wgpu authority consume only that packet and never re-query mutable ECS topology; a second view/target in the packet, generation drift, or repeated acquire fails before touching the surface. Winit physical input resolves through `ActionMap` and lowers into U6's semantic ingress. Focus loss clears retained action state and emits release/stop; focus regain ignores stale repeats or held keys until a new press. Sprite queueing fixes visible transparent order before adjacent material batching. UI batches retain effective clips, split when clip changes, reject non-finite coordinates, and encode clamped target-space scissor state. A game-owned desktop projection reads authoritative state only; it cannot mutate combat/wave outcomes outside commands and fixed-safe-point Retry. Any parented spatial/visibility content requiring unimplemented propagation rejects before publication. U11's retirement result is integrated into Winit Stop/Quit without moving target ownership into U5.
- **Driver Interface gate:** U13 replaces the broad U5 trial `RuntimeDriverScope::world_mut` escape hatch with typed, state-gated platform operations for normalized events, elapsed time, redraw, target retirement, and close integration, or proves an equivalently narrow scoped carrier. It must not expose `World`/`App` access or freeze generic mutation as the Platform Adapter contract. The pre-1.0 implementation may use the `#[doc(hidden)]` and explicitly removable `__RuntimeDriverPort` solely as cross-crate, resource-local plumbing: Rust visibility requires the trait to be public for sibling crates, downstream code can opt in only its own resource types, orphan rules prevent it from adding access to engine-owned resources, and the operation receives no ambient `World`. This carrier is excluded from compatibility promises and may be deleted without migration. OQ-038 still requires a second production Platform Adapter before choosing a shared public trait, static-generic carrier, or other reusable driver shape; OQ-017 continues to own advanced raw event access.
- **Desktop flow:** U24 prepares the desktop-profile `RuntimeRecipe`, begins one start attempt, and automatically continues into the fixed wave after publication. The first workspace desktop binary remains a console-subsystem binary and still resolves its project from the source checkout; U7 owns executable-relative packaged startup. Startup/runtime failures emit one bounded privacy-safe diagnostic to stderr and exit nonzero; no pre-Running loading UI or persistent failure UI is promised. WASD moves. Manual playability feedback may tune the committed shared scene values, but presentation code never mutates authoritative gameplay and every such tuning updates the raw-App/headless baselines plus desktop parity together. Completion or defeat freezes authoritative gameplay and shows distinct game-terminal geometry. Health current/max changes HUD geometry through empty death; progress follows the defeated/planned contract. Enter lowers through the same semantic-command path as headless adapters and is accepted only on a game-terminal just-pressed edge, with at most one Pending reset per `WaveRunGeneration`. The first fixed Consume records the Pending request without mutation; the next fixed safe point, before ordinary simulation, uses the captured bounded game template plus scene-identity replacement and publishes a new wave-run generation at run tick one. `RuntimeGeneration`, window, GPU/device, and services remain unchanged; duplicate, Running-state, or already-Pending requests reject. Runtime/reset failure leaves the old scene generation authoritative and never reports Applied. Runtime/reset failure and Quit request bounded Stop plus mandatory local surface retirement. Window/provider release waits for SurfaceRetired. If close evidence contains `DeadlineExceeded`, the Host emits `shutdown_timeout`, retains the CloseIncomplete owner without claiming Stopped until process teardown, and exits nonzero after the window is safely released; non-deadline participant failures keep their distinct runner/cleanup classification, and successful shutdown exits normally.
- **Test scenarios:** Covers AE6, AE9, AE14, and AE15. The window slot accepts an explicitly configured `WindowPlugin`; duplicate/missing slot, missing capability/service, or conflict rejects before mutation, and the production desktop recipe retains the same resolved plugin identity while matching headless content digest/source hash and independently retaining then releasing its lineage-bound lease. A Winit driver probe proves every frame, Pause, fault, and Stop passes through `RuntimeInstance`; static and dynamic tests reject a platform-held raw `&mut App` or direct `App::run_once`. A supported native platform smoke executes the public Winit event-loop runner, requests exit, retires every owned target before provider release, preserves combined runner/teardown failures, and proves the final runtime result. A captured physical action stream and direct headless stream finish at the same tick/snapshot. Focus loss stops movement and focus regain needs a new press. Local command rejection faults both paths. Overlapping transparent sprites with alternating materials preserve painter order while batching only adjacent compatible items. Effective UI clips survive queue/backend conversion; feature-enabled production readback proves clipping, target clamping, and non-finite rejection. The supported packet acquires/submits/presents once. Injecting a second view after extraction cannot affect the current packet and causes the next packet to record one rejection with zero acquire/submit/present; stale generation and repeated acquire also perform zero surface work. HUD batches prove health, death, progress, and game-terminal geometry. Startup/runtime failures are captured on stderr, and the desktop product probe is reaped by a parent-owned hard deadline. Direct headless Retry plus physical Enter prove one semantic reset path; Running and duplicate/Pending cases reject, successful and repeated resets increment `WaveRunGeneration` while preserving one `RuntimeGeneration`. Normal Quit, deadline shutdown-timeout Quit, and non-deadline close failure prove retirement order, truthful non-Stopped state, retained ownership until process teardown, classification, and exit code.
...
- **Verification:** Independent desktop/parity tests, the supported native public-event-loop smoke, window-slot configuration, root render regressions, packet-generation/admission counters, stderr/exit tests, production-path pixel checks, and all three named desktop example builds pass; the flow is manually playable on Windows. Public-surface and dependency audits find no private hook, reference-game branch, test renderer, mutable-world topology re-query after extraction, per-camera acquire/present, or target-owned submit. U7 owns hosted software-adapter artifact smoke and records unsupported macOS lifecycle claims rather than inferring them from Windows/Linux.

### U14. Record the First-Playable Product Baseline

- **Goal:** Record a small reproducible baseline and name the next product bottleneck after Nara's headless and desktop workflows are already playable.
- **Requirements:** R2-R3, R22's pre-candidate subset, and R32.
- **Dependencies:** U6's authoritative headless wave, U13's desktop first playable, and U15's disposable Windows/Linux feedback path.
...
- **Files:** a small cross-platform measurement helper under `reference-game/tools/`, its focused tests, `tests/measurement_policy.rs`, and `docs/benchmarks/reference-game-first-playable-baseline.md`.
- **Approach:** Measure the actual public workflow, not a synthetic harness: one data edit/reload, one structural Rust edit/rebuild/fresh runtime, clean checkout-to-headless success, clean checkout-to-manually-playable desktop success, public production coverage, frame-time P99, process memory, and cold/incremental build time. For the admitted render path, also record packet batch/instance/retained-byte counts plus observed packet clone/allocation work so the owned transfer cost is visible before adding a budget or changing Host placement. Record environment, mechanism, sample count, failures, and non-claims in one compact document. Use isolated temporary workspaces and a cross-platform helper where automation adds value; do not require canonical envelopes, Git ancestry policy, transitive source manifests, or release-grade provenance for local evidence. U20 may rerun or expand these measures against exact candidates.
- **Test scenarios:** The helper refuses the active dirty checkout as a measurement subject, preserves failed samples, separates incompatible environments, and leaves the source worktree unchanged. A valid run records the exact public entry point, mechanism, environment, count, raw numeric samples, and render-packet cost counters without copying pixel payloads merely to measure them. Private engine hooks or undocumented setup make public coverage fail. Missing desktop or headless success produces a named bottleneck, not a fabricated Continue verdict.
- **Verification:** The compact baseline is reproducible from committed instructions on one supported Windows environment and one disposable Linux CI environment where applicable; both public paths succeed, measurements name their limits, and U14 adds no trust-ingestion or benchmark framework. U7 may proceed from the working product regardless of whether a metric becomes the next optimization target.
...
- **Dependencies:** U5's code-first runtime evidence, U26's pre-Host reference, U24's concrete Host/candidate and integrated reversal evidence, U17's Editor command/view and stop-first evidence, U13's desktop driver/close evidence, and U19's ADR-governance validation.
...
- **Approach:** An independent reviewer first evaluates ADR 0084 against exact U5 candidate/runtime, fault, driver, and close evidence without requiring the outer product Host. That review must decide whether process-global execution exclusion is a supported product invariant, a temporary Bevy/resource safety constraint, or an unnecessary restriction; it must also reconcile the plan registry and World-local runtime registry instead of accepting two behavior authorities because the current types exist. It separately evaluates ADR 0082 against U26/U24/U17/U13 content/plan lineage, parent lifetime, start-attempt, ordinary-author surface, Editor, and desktop Host evidence while treating the runtime result as an explicit dependency rather than a shared verdict. Each ADR becomes Accepted, Rejected, or remains Proposed on its own metrics. The review then evaluates the resulting pair: Accepted/Accepted may unblock U20 only when integration evidence is compatible; an Accepted/Rejected or Rejected/Accepted result preserves the supported decision but requires an Accepted successor for the rejected product requirement plus rerun of affected evidence; Proposed leaves the corresponding authority blocked. Authority/foundation text changes only for the decision actually accepted. This review does not add code, rename types from preference alone, or infer desktop/Editor parity from headless evidence.
- **Trust gate:** U23 either depends on an Accepted native-project-code trust decision or explicitly limits ADR 0082/0084 acceptance to already compiled, Host-trusted code-first and RGF paths. It must not imply that opening project data authorizes Cargo build scripts, proc macros, native packages/importers, or in-process Play; broader Editor/package activation remains blocked on OQ-031 or an Accepted successor.
- **Test scenarios:** Deleting a required evidence set, citing either Proposed/Rejected ADR as current authority, accepting an ADR with a failed metric, hiding a mixed result, omitting the compatibility verdict, or updating unrelated foundation authority fails governance checks. The matrix includes sequential and overlapping RuntimeInstance attempts, process-global authority contention, fresh-runtime reconstruction, and divergence between plan and World registries; it records which cases must reject and why. Complete runtime evidence may accept ADR 0084 while ADR 0082 remains Proposed/Rejected, and vice versa when its dependency is represented by an Accepted successor. A rejected product requirement without an Accepted successor blocks U20. Complete compatible evidence permits Accepted/Accepted; a coherent contradiction may reject either or both with explicit successor linkage.
...
- **Approach:** Leave only `CoreStage::TaskUpdate` in `nara_app` and bounded execution mechanics in `nara_tasks`. Move `AssetTaskUpdateSet::{Poll, ResolveSourceChanges, SpawnJobs, ApplyResults}` and ordering into `nara_asset`; watcher/image adapters join through asset-owned public vocabulary. Replace the watch callback's unbounded channel with explicit bounded admission; overflow, disconnect, and callback translation failure update structured diagnostics and pressure counters instead of disappearing. Characterize the first playable's ready-prefix and predecessor behavior before moving symbols, then delete old aliases and ownership paths.
- **Test scenarios:** Poll captures one ready membership/prefix at entry; eligible predecessor-unblocked results apply in the same frame; later-ready or missing-predecessor work waits; stale/superseded outcomes retire; watcher queue capacity rejects or coalesces according to a declared policy, and overflow/disconnect/translation failure is observable without blocking the callback; watcher coalescing and image reload preserve last-good behavior; the reference-game asset/task flow produces the same authoritative result before and after the ownership move; `TaskPlugin` configures no asset sets.
...
- **Draft-governance scenario:** The architecture index classifies every top-level design harness, appendix, and guide; no non-normative draft is presented as current ADR authority or active execution order, and every `needs-rebaseline` document names its activation owner.
...
- **Dependencies:** U13's desktop first playable and U15's minimum hosted CI skeleton. U8, U14, U17, and U19 do not block candidate construction but must land before U20 approval.
...
- **Requirements:** R2-R3, R22, R32-R33, and R37; AE19 and AE22.
- **Dependencies:** U7's immutable candidates, U8's ownership cleanup, U13's desktop evidence, U14's lightweight first-playable baseline, U17's editor Save/Play integration, U19's governance validation, and U23's compatible Accepted decisions or Accepted successors for every required ADR 0082/0084 product authority.
- **Entry trigger:** Headless, desktop, editor, and standalone candidate workflows exist, but the complete pre-publication diff, independent Rust-author public-API clean-room journey, exact candidate delivery, artifact size/startup, and final publication/next-slice decisions remain unreviewed.
- **Files:** new `.github/workflows/reference-game-evidence-ingest.yml`, new
  `reference-game/tools/ingest-evidence.ps1`, expanded
  `reference-game/tools/{measure-headless-iteration,measure-desktop-product}.ps1`,
  `tests/{measurement_policy,ci_policy,evidence_envelope}.rs`, new
  `docs/benchmarks/{reference-game-baseline,reference-game-evidence-review}.md`,
  the normalized-artifact schema at
  `docs/benchmarks/data/envelope/v1/normalized-evidence.schema.json`, the first normalized artifact
  at `docs/benchmarks/data/normalized/v1/reference-game-final-evidence.json`, the first canonical
  approval record at
  `docs/benchmarks/data/approvals/v1/reference-game-pre-release.json`, restricted raw-log artifact
  references, and focused verification records. A correction allocates `v2` or later paths; it
  never overwrites a prior normalized artifact or approval record.
- **Approach:** First run simplification and bug/regression-focused review over the complete pre-publication diff, resolve P0/P1 findings, remove abandoned attempts, and repeat affected gates. Then run the Rust-author public-API clean-room journey below with an independent runner, isolated permissions, declared entry state, allowed edits, completion record, and failure result. Any API/docs fix discovered by the journey returns to full pre-publication review and repeats the affected journey. Only after the review and journey pass without further source changes does U20 rerun U7's read-only pipeline from the exact reviewed product source commit to create the final publishable candidate; all earlier and clean-room candidates remain lineage only.
- **Rust-author public-API clean-room journey:** A fresh runner with no implementation context starts from the committed reference-game project and public documentation, runs the headless wave, makes one temporary persistent-component source edit, configures one supported plugin slot through the documented Rust/project surface, exercises Editor Play, and produces a clearly non-publishable trial candidate. This journey may edit the documented Rust/project inputs required by AE19 but may not use private dependencies, engine-only hooks, infrastructure IDs, or implementation context. It records runner class, allowed edits, task time, result, and every undocumented intervention.
- **Clean-room isolation:** Human and agent-assisted runs use an isolated HOME, trust-separated caches, sanitized child-process environment, no secrets/OIDC, no SSH/Git/Cargo/release credentials, and no persistent checkout credentials. The journey uses its own evidence envelope and cannot reuse a publishable artifact identity.
- **Final candidate and approval:** Before accepting final bytes, verify repository owner/name, workflow path and reviewed definition SHA, allowlisted event/trigger chain, protected source ref, run ID/attempt, product source commit, immutable artifact IDs, sizes, retention policy/deadline, and digests. Preserve U14's raw lineage and apply U22's digest-bound source/environment rules per metric: reuse is permitted only when every required environment field remains in its predeclared equivalence class; an out-of-class, ambiguous, shared engine/build/config, or unknown change reruns the full affected suite. Add final candidate size/startup and Editor workflow results; do not re-collect already valid U14 metrics merely to change their ranges. One canonical bounded approval record contains the Publish/Redirect/Stop decision, next-slice rule, reviewed product source commit, protocol digest, journey identity, and exact final candidate identities/digests. After that record is committed and reviewed, U20 completion evidence records its protected-default-branch approval commit, Git blob object ID, and independent SHA-256 byte digest; changing any field creates a new U20 approval version rather than mutating the published authority.
- **Evidence transfer:** The untrusted clean-room/candidate jobs emit only U22 envelopes and
  restricted raw-log artifact references. The
  `.github/workflows/reference-game-evidence-ingest.yml` job downloads independently expected
  identities under explicit size ceilings. Its reviewed definition pins a protected-default-branch
  commit, repository-relative path, Git blob object ID, and SHA-256 bytes for both
  `reference-game/tools/ingest-evidence.ps1` and
  `docs/benchmarks/data/envelope/v1/normalized-evidence.schema.json`; dispatch and collector input
  cannot override any of those values. The job fetches and verifies only those exact blobs before
  executing the helper. The helper verifies the outer transfer digest, preflights any archive table,
  parses without shell interpolation against the pinned schema, validates all bound
  identities/digests/environment fields, and emits only one immutable normalized artifact plus
  digest. The job is read-only and secret-free,
  has no OIDC, checkout credential, repository mutation, Release mutation, or publication
  authority, never checks out the product tree, and executes no candidate or product code. It
  rejects unknown or value-bearing sensitive/secret fields, symlinks, special files, aliases, path
  escape, unexpected entries, forged fields, and canaries. A separate reviewed human/PR step may
  commit only the already normalized digest-bound record at the versioned canonical path and never
  parses collector-controlled bytes.
- **Test scenarios:** The helper rejects a final candidate built before the last required journey/review commit; an owner/workflow path/definition SHA/event/trigger/ref/run/attempt/artifact/digest/size/source mismatch; fork, `pull_request_target`, untrusted `workflow_run`, or unpinned reusable workflow provenance; expired or insufficient retention; stale samples without U22's required mapped/full rerun; rewritten parent samples; post-implementation range/map/environment-class changes; unsafe temporary roots; insufficient raw samples; mixed cold/warm populations; hidden failures; percentile-only summaries; and missing or out-of-class OS/runner-image/toolchain/CPU/GPU/software-adapter/build-profile/tool-version fields. A source-and-environment-equivalent run may reuse valid lineage; multi-match invalidation takes the union, shared engine/build/config or out-of-class environment changes rerun the full affected suite, and unknown changes rerun all while preserving ancestors. Clean-to-desktop includes dependency fetch/build, authorized content snapshot/profile RuntimeRecipe, window publication, and a documented manual-playable observation point. The clean-room journey fails on host credentials/secrets/OIDC, inherited SSH agent or Git/Cargo auth, unsanitized credential variables, persistent checkout credentials, trust-shared caches, candidate-code execution in the evidence-upload step, private knowledge, undocumented commands, role-forbidden edits, private dependencies, engine-only hooks, reuse of a publishable artifact identity, or any resulting fix not followed by full review and a fresh affected journey plus final-candidate run. Publish fails while any P0/P1 review item, frozen-range gate, unit gate, U23 authority/compatibility decision, final candidate identity/rerun, journey result, or U8/U17/U19 evidence remains incomplete.
- **Evidence-transfer scenarios:** Covers AE22. The ingestion job has read-only permissions and no secrets, OIDC, checkout credentials, repository/Release mutation token, or publication environment. A missing, unprotected, unreachable, dispatch-overridden, wrong-path, wrong-blob, or SHA-mismatched helper/schema pin rejects before parser execution. It also rejects every envelope/archive limit+1, traversal/alias/link/special-file, unexpected entry, forged provenance/environment field, digest mismatch, command-injection string, and path/username/proxy-URL/credential canary without repository mutation or candidate/product-code execution. A valid final-candidate envelope remains bound to the exact reviewed candidate and cannot substitute local U14/U24 product evidence; only its normalized immutable output and digest may cross into the separately reviewed commit step.
- **Approval-record scenarios:** A changed decision, next-slice rule, product source commit, protocol digest, journey identity, candidate field, approval commit, blob object ID, or approval-record SHA-256 invalidates U20 completion and cannot be consumed by U21. The same canonical bytes reproduce the recorded blob and SHA-256 identities; a later correction creates a new approval version and reruns affected review rather than overwriting the old authority.
- **Verification:** Baseline and evidence-review documents contain the reviewed product source commit, independent journey identity/result, final trusted-run/artifact identities, sizes, retention deadline, candidate and protocol digests, invalidation lineage, runner class, credential-isolation policy/result, hardware, sample count, raw-data locations, P50/P95/P99, workflow success/time, frozen comparison rules, fallbacks, and non-claims; the complete pre-publication diff has no unresolved P0/P1 finding or abandoned code. One canonical Publish/Redirect/Stop approval record and next-slice rule exist, and completion evidence pins its protected-default-branch approval commit, blob object ID, and SHA-256 bytes. Measurement, ingestion, and candidate rerun leave the active worktree unchanged; ingestion has no mutation authority; no candidate code executes in a credential-bearing step.
- **Architecture handoff:** Before U21, update the architecture index, foundation summary, runtime-composition harness status, and implementation ledger against the reviewed release commit. Record actual owners/symbols, Accepted ADRs, unresolved OQs including OQ-007/OQ-015/OQ-031/OQ-033/OQ-039 through OQ-043, every design draft that still needs rebaseline, and any legacy ADR template debt that still needs a substantive owner review. Using the U14 Rust iteration baseline, U17 Editor Host evidence, U7 standalone candidate, U20 clean-room results, and any admitted bounded OQ-007 feasibility tracer, record OQ-007 as either still deferred or admitted to a separately planned C#/.NET Trial. The latter requires the Trigger plus a pinned tracer revision, result, and passing stop-condition verdict; absent or failed feasibility evidence can only leave OQ-007 deferred. This is a non-blocking next-slice decision: it cannot change the current Publish/Redirect/Stop verdict, activate implementation directly, or permit empty boilerplate sections.
...
- **Dependencies:** U20 must record Publish for its exact final trusted-run/artifact identities and digests, with retention remaining beyond the publication window, and pin the canonical approval record by protected-default-branch commit, Git blob object ID, and SHA-256 byte digest.
...
- **Files:** `.github/workflows/ci.yml`, new `reference-game/tools/verify-release.ps1`,
  `tests/ci_policy.rs`, release documentation,
  `docs/benchmarks/reference-game-evidence-review.md`, U20's pinned versioned canonical approval
  record beginning at `docs/benchmarks/data/approvals/v1/reference-game-pre-release.json`, and
  focused release verification records under `docs/knowledge/engineering/`.
- **Approval authority:** The protected U21 invocation pins U20's approval commit, blob object ID, and
  SHA-256 bytes. Before reading a candidate identity or creating a draft, the publisher fetches only
  that exact blob from the pinned commit, proves the commit is reachable from the protected default
  branch, checks both object and byte digests, parses the bounded canonical record, and requires its
  decision to be Publish. Candidate identities, reviewed product source, protocol digest, journey
  evidence, and next-slice rule come only from that record. Any changed or substituted record returns
  to U20; U21 never reconstructs approval from mutable prose or independently valid artifacts.
- **Approval pin scenarios:** A missing or unreachable approval commit, wrong path/blob object ID,
  SHA-256 mismatch, non-Publish decision, changed candidate/source/protocol/journey field, or record
  first committed after the reviewed publisher input prevents draft creation. Draft-upload and
  release-finalize both recheck the same triple; a provenance-valid candidate not named by that exact
  record remains ineligible.
...
# RGF-U22
cargo nextest run --locked -p nara --features serde --test measurement_policy --test evidence_envelope --test-threads=1
...
# RGF-U28
cargo nextest run --locked -p nara_app --test schedule_compatibility --test-threads=1
cargo nextest run --locked -p nara_gameplay --lib --test-threads=1
cargo nextest run --locked -p nara --test schedule_extension_contract --test-threads=1
cargo nextest run --manifest-path reference-game/Cargo.toml --locked --test public_surface --test-threads=1

# RGF-U29
cargo nextest run --locked -p nara_ecs -p nara_ecs_derive -p nara_reflect -p nara_reflect_derive -p nara_scene --test-threads=1
cargo nextest run --locked -p nara --test scene_component_composition --test-threads=1
...
# RGF-U13
cargo nextest run --locked -p nara_render -p nara_winit -p nara_input -p nara_gameplay -p nara_sprite_render -p nara_ui_render --test-threads=1
cargo nextest run --locked -p nara_render_wgpu --features sprite-submitter,ui-submitter --test-threads=1
cargo nextest run --locked -p nara --test schedule_extension_contract --test-threads=1
cargo nextest run --locked -p nara --features runtime-2d,runtime-ui,serde,desktop-winit,render-wgpu --test project_content_boot --test project_runtime_boot --test project_host_boundary --test runtime_driver_boundary --test runtime_instance --test plugin_composition --test-threads=1
cargo nextest run --manifest-path reference-game/Cargo.toml --locked --features desktop --bin desktop --test desktop_flow --test desktop_parity --test desktop_render --test first_wave --test headless_snapshot --test plugin_composition --test public_surface --test-threads=1
cargo run --locked -p nara --no-default-features --features desktop-winit,render-wgpu --example window_surface_retirement_smoke
cargo run --locked -p nara --no-default-features --features desktop-winit,render-wgpu --example window_surface_retirement_smoke -- --drop-backend-before-exit
cargo check -p nara --locked --no-default-features --features desktop-winit,render-wgpu --example windowed_clear
cargo check -p nara --locked --no-default-features --features runtime-2d,desktop-winit,render-wgpu --example windowed_sprites
...
# RGF-U14
cargo nextest run --locked -p nara --test measurement_policy --test measurement_helpers --test-threads=1

# RGF-U17
cargo nextest run --locked -p nara_fs -p nara_tooling -p nara_tooling_egui --test-threads=1
cargo nextest run --locked -p nara --test editor_persistence --test scene_play_mode --test workspace_play_runtime --test-threads=1
...
# RGF-U20
cargo nextest run --locked -p nara --test measurement_policy --test evidence_envelope --test ci_policy --test-threads=1
...
pwsh -File reference-game/tools/ingest-evidence.ps1 -VerifyPolicyOnly
...
- U22's measurement vocabulary and bounded external-evidence envelope remain available without blocking product work. U26 retains the committed manual task and compact owner/fault record before U24; U24 proves task-equivalent authoritative output and a smaller ordinary caller surface before U6. Concrete U6 correctness or safety failures block U13, while U14 records a lightweight baseline only after both public paths work.
- U28 is complete before U6 adds or publishes reference-game scheduling dependencies; its external
  fixture imports no first-party system function and proves the exact four anchors' participation,
  deferred, skip, App/domain fault, and cleanup behavior plus absent/cross-schedule non-guarantees;
  seal rejects disabled automatic deferred insertion, reasserts final deferred application, and the
  conformance oracle rejects ignore-deferred claims without claiming a total peer order.
- U12 and U29 are both complete before U26 first materializes content into a target `World`: U12
  certifies only bounded document/schema truth, while U29 proves static provider plus per-apply
  target-World eligibility across World-registered hooks, all five lifecycle events, and all four
  observer scopes through fresh-target global/component checks plus existing-target entity checks
  before persistent mutation, and preserves a runtime-only Bevy fixture.
...
- U23 independently reviews ADR 0084 against U5 evidence and ADR 0082 against U26/U24/U17/U13 evidence, records each decision separately, then proves compatibility; U20 requires every product authority to be Accepted or replaced by an Accepted successor with affected evidence rerun.
- U20 runs simplification and bug/regression-focused review over the complete pre-publication diff, resolves P0/P1 findings, repeats affected gates, removes abandoned attempts, completes an independently run Rust-author public-API journey in a disposable credential-free runner/VM, and pins the canonical approval record plus exact final run/artifact identities before U21. Journey failure prevents Publish; candidate code never executes during read-only evidence ingestion, and that job holds no repository or Release mutation authority. OQ-037 project creation remains outside this release gate until external adoption evidence admits a later slice.
...
- U22 preserves the landed measurement vocabulary, privacy classifications, invalidation rules, and bounded external-evidence envelope without making local product work depend on a benchmark or ancestry framework.
...
- U28 proves from a renamed-dependency external fixture that only documented semantic schedule/set
  anchors form supported registration, membership, and set-ordering dependencies, with explicit participation/deferred/skip/App-domain-fault/
  cleanup behavior, seal-time automatic deferred validation and final deferred reassertion, explicit
  ignore-deferred opt-out from compatibility, and no total-order, system-function, private-set,
  incidental-order, absent-target, cross-schedule, or new scheduler dependency.
- U29 proves canonical-v1 persistent composition equals the explicit post-prefab stable-ID set,
  rejects static required-component/intrinsic-hook metadata at provider freeze and late required/
  dynamic-hook metadata or matching lifecycle observers across all five lifecycle events and four
  observer scopes through fresh-target pre-allocation global/component checks and
  existing/reserved-target pre-mutation entity checks under one exclusive borrow, preserves runtime-only and
  post-publication Bevy behavior, keeps hook introspection private to `nara_ecs`, and adds no preset
  or component-dependency API.
- U12 proves a public authorized manifest -> bounded path-addressed startup dependency closure -> leased immutable document/artifact/digest `ProjectContentSnapshot` from random cwd/home, consuming U4's provider/schema input and carrying one opaque project lineage/fingerprint, including queue/in-flight and byte limit+1 cases, structured StableId and unsupported hierarchy rejection, shared Arc-backed image-payload identity with instance-local asset state, and no target-World topology claim, whole-root scan, ambient reopen, duplicate payload, native binding/runtime plan/service/source authority in the snapshot, partial publication, or speculative universal host API.
- U5 proves a manifest-free code-first `RuntimeCandidate` -> `RuntimeInstance` path around one sealed unstarted `App`, explicit registered obligation ownership, caller-owned arbitrary resources, App-owned exact complete StepFixedTick, qualified sticky faults, runtime-only driver authority, observable controls, distinct generations, and bounded retryable close without owning project content, start-attempt policy, reconstruction, or a second execution model; the source-bound U5 review has no unresolved P1, ordinary error propagation cannot lose retirement ownership, and panic containment remains unadmitted. U24 owns the permanent concrete action and author-facing concept check; U13 owns the native public-event-loop smoke.
- U26 retains the committed manual U4-plan/U12-snapshot/U29-eligible-apply raw-App task plus a concise human-readable concept, fault, owner, and shutdown record; no policy engine or evidence envelope is created for local comparison.
- U24 proves the concrete root Host recipe/start transaction and the same-task reversal check: lineage/schema matching, one registered obligation ledger before fallible work, sealed-App transfer into U5's unpublished candidate before startup, U29's repeated post-flush exclusive World guard before scene allocation, guarded acquire-then-fail and cancellation retirement, truthful `RetirementIncomplete` ownership, registry/service/scene/startup ordering, first fixed tick from committed project content, equivalent authoritative output to U26, a small ordinary caller surface, and one atomic infallible compare-and-consume publish-and-promote boundary without source reopen, stale/duplicate/late epoch publication, an intermediate promoted owner, or a public universal Host/factory Interface.
...
- U14 records a compact reproducible baseline for the already-playable headless and desktop public paths, including data and structural Rust edit-to-result, clean-start success, public production coverage, frame-time P99, memory, and build time with explicit environment, sample-count, failure, and non-claim metadata; it names the next bottleneck without adding a trust-ingestion, ancestry, or benchmark framework and does not block U7 on a fabricated threshold verdict.
- U17 replaces the tooling bare Play `World` with a concrete Editor Host-owned recipe/start/runtime owner and UI-neutral tooling commands/views, deletes `MarkSaved`, and proves receipt-backed known-schema Save, explicit dirty Close/Exit Save-Discard-Cancel, reopen from persisted bytes, `NoActiveDocument`, PreparingPlay Cancel without an attempt, Starting Cancel through owned RetiringPlay, and the public patch -> Save -> close -> reopen -> one-click recipe preparation -> Play -> Pause Applied -> StepFixedTick -> Stop -> fresh Restart flow. It also proves explicit safe-point runtime edit versus Apply Changes semantics, complete egui state/control coverage, and one pending workspace intent without freezing an in-process topology or creating a root/tooling dependency cycle.
- U13 proves U24 desktop-profile startup plus same-plugin window configuration, Winit-owned driving through `RuntimeInstance` rather than raw `App`, desktop/headless input parity, focus recovery, one immutable generation-stamped window/target/view packet, pre-acquire rejection of extra/drifted topology, fail-closed parented spatial content, painter-order-safe sprite batching, backend UI clipping, game-terminal HUD, stderr failure contract, truthful shutdown-timeout exit, precise Retry/Quit, and the manually playable Windows flow without a desktop-only authoritative gameplay fork; any playability tuning remains shared committed scene data proven by raw-App, headless, and desktop evidence.
...
- U20 waits for U7/U8/U13/U14/U17/U19 plus U23-compatible Accepted authority, reviews the complete pre-publication successor, completes an independent Rust-author public-API journey plus clean-to-desktop, slot-configuration, frame-time, memory, build, candidate, editor-workflow, and public-coverage measurements in a credential-free disposable runner, accepts collector data only through identity-bound budgeted redacted envelopes in a non-executing read-only ingestion job with no mutation authority, reuses U14 evidence only within U22's per-metric environment-equivalence classes, reruns all source/environment-invalidated protocols with preserved lineage, pins one canonical Publish/Redirect/Stop record by approval commit, Git blob object ID, and SHA-256 bytes against the exact final candidate plus next-slice rule, and records without blocking that verdict whether OQ-007 remains deferred or may enter a separately planned C#/.NET Trial; admission requires its Trigger plus a pinned passing feasibility-tracer result, while absent or failed evidence preserves deferral.
- U21 first clears independent P0/P1 review of its final permission-bearing workflow commit, then consumes only U20's pinned approval commit/blob/SHA triple and publishes no rebuild: no-checkout draft-upload/finalize jobs revalidate the exact record plus repository/workflow-definition/event/trigger/ref/run/artifact identity and release only its approved digests before retention expiry under an authorized immutable `vX.Y.Z` pre-release tag, while read-only draft and anonymous public headless/desktop smoke gate publication and announcement; any record change or later defect starts a new U20/U21 version.
