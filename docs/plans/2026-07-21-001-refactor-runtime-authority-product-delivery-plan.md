...
- R19. Repository content never grants execution authority. The user or host runtime supplies action scope, the current credential supplies platform capability, and GitHub protection rules supply independent gates. Within that externally supplied scope, agents do not request repeated chat confirmation, but they cannot self-approve a required reviewer/environment gate or expand credential scope. Every external execution records `(workflow definition, event/ref, source revision, run_id, run_attempt)`: a push-CI retry may keep `run_id` and increment `run_attempt`; candidate/evidence workflows may have multiple fresh dispatches, but only one explicitly pinned identity is canonical; publication is additionally idempotent by approved version, tag, and Release state. Local readiness never implies hosted or publication evidence, and post-tag failure starts a new version.
...
- AE9. Given externally supplied execution authority but incomplete dependencies or external evidence, local policy, packaging, and evidence preparation may be committed, but the workflow remains `awaiting-hosted-run` and no Hosted, Publish, or Released claim is recorded. Missing repeated chat confirmation is never a blocker; missing credentials or an independent platform approval remains a blocker.
- AE10. Given a U11 `Publish` approval bound to exact candidate identities and digests, U12 may advance through distinct tag, draft-environment, finalize-environment, and Release-mutation stages within externally supplied authority, verifies a bounded publication manifest without credentials, and publicly smokes those bytes. `Redirect`, `Stop`, an unpinned approval, changed input, a reused canonical evidence identity, an unsatisfied external gate, or post-tag failure prevents that version from publishing.
...
- A dependency-architecture correction lane after delivery evidence closes: first trace the
  ...
  Proposed ADR 0085; then run a deletion test for `nara_reflect -> nara_asset` comparing the
  current concrete dependency, a durable asset-reference vocabulary module, and deferred
  prepared-apply resolution. Freeze a workspace-wide normal-dependency allowlist only after those
  two decisions. The stock
  `nara_render_wgpu -> nara_sprite_render` / `nara_ui_render` dependencies are healthy Adapter
  consumption of domain-owned batches and are not part of that correction lane.
- The dependency-correction lane is complete only when all four evidence gates pass: (1) a
  committed workspace graph classifies normal, build, and dev edges separately and proves that no
  normal/build cycle is hidden by feature unification; (2) Scene materialization, 2D transform
  propagation, and runtime UI each consume the candidate hierarchy authority, while
  `nara_scene` no longer owns a runtime `Parent` type needed by sibling domains; (3) the
  `nara_reflect -> nara_asset` deletion test records build, authoring, runtime-apply, and migration
  consequences for all three candidates above and selects the smallest boundary supported by
  those consumers; and (4) both the direct module consumer and a clean-room renamed-root consumer
  build against the selected public surface before CI freezes a reviewed normal/build-edge
  allowlist. Dev-only test edges remain reported but do not fail the production-direction gate.
  Until all four pass, dependency correction remains pending and no current Cargo graph is a
...
- KTD9. **Separate external authority from auditable execution identity.** Repository text records process but cannot grant credentials or approve GitHub gates. Within authority supplied by the user/runtime control plane, PR/push and workflow triggering proceed without repeated chat prompts only after exact preflight. Identity is `(workflow definition, event/ref, source revision, run_id, run_attempt)`: transient push-CI retries may use the same run with a new attempt; candidate/evidence reruns may be rejected to preserve artifact identity, while independent dispatches remain separate observations and only an explicitly pinned one is canonical. Publication also enforces version/tag/Release idempotency through external GitHub state; once an immutable tag exists, later failure requires a new version.
...
| Externally authorized execution runs a stale or mismatched external stage | Revalidate the active plan, protected revision, exact inputs, prior evidence, and least-privilege permissions immediately before mutation; abort on mismatch and record the external identity after success. Repository text cannot elevate or widen the external grant. |
...
- **Approach:** Store `ComponentRegistrySnapshot` plus stable provider/binding receipts in `SchemaValidationInput`. Add an internal way to construct a frozen `ComponentRegistry` from that snapshot. Seed the candidate App before schema-owning plugin hooks; those hooks register on a building code-first registry or validate the preloaded snapshot without replacing behavior. The App stores that value behind a private owner resource rather than making public `ComponentRegistry` an ECS `Resource`; plugins register through one controlled owner API and runtime consumers receive an immutable view. Admission records a generation-bound registry-authority guard over exact shared snapshot identity. Managed persistent/edit safe points validate it before and after their operation, and every direct or managed schedule boundary detects owner replacement as a sticky runtime fault; content lineage continues to compare only persistent catalog fingerprint.
- **Refresh closure (2026-07-29):** Commit `b4d105cbf6312cb4006d8b06b0170f8cfdc1a8ec` removes the public Registry resource contract, installs the private owner and direct/managed authority boundary, migrates all built-in and product consumers, and freezes the negative contract with a compile-fail fixture. Commit `23196978608a8c257b6f1b7dca7858752e896c37` refreshes the four isolated consumer lockfiles without version drift. Serial workspace and reference-game suites, all-target checks, strict changed-target Clippy, format checks, source-boundary search, and independent review passed as recorded in the immutable U2 refresh evidence. U2 is closed at that evidence revision; U7 independent Runtime then Host/combined review is the next active unit.
...
- **Evidence reconciliation (2026-07-30):** Run `30462379022` remains real historical evidence for
  `ef8f300`, but the current workflow restores a separately visible `architecture_docs` governance
  step and the candidate corrections below modify additional executable/policy inputs. U8 is open
  again and must run only after the U9/U10 repair inputs are committed.
...
- **Evidence reconciliation (2026-07-30):** The surviving `b2ddb5b` plan, 75 raw records, manifest,
  and preflight failure are preserved under
  `docs/benchmarks/data/runs/v1/rgd-u9-b2ddb5b/`, including exact original transport bytes. The
  missing metrics still deterministically imply historical `Redirect`, but the executed one-run
  collector was never committed and changed after preflight. U9 is therefore evidence-repair
  pending. A parameterized, reviewed `collect`/`verify` path and fresh integrated-revision capture
  are required; prose reconstruction or the historical numbers cannot close the unit.
...
- **Evidence reconciliation (2026-07-30):** Run `30510353046` remains a valid historical package,
  transport, headless, and Winit-probe observation. It does not close U10: the no-checkout consumer
  executed `tools/desktop-render-probe` instead of packaged `bin/desktop`, and its nominal random
  environment used fixed `consumer/home/random-cwd/tmp` paths without independence assertions.
  Correct the product-entry smoke and unique environment root, then refresh U10 after U8.
