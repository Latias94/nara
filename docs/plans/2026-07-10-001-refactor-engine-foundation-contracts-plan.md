...
- R14. Scene, prefab, patch, asset metadata, artifact, schema catalog, and editor recovery-journal files must use a common envelope or explicitly versioned record header, publish a `kind x format_version` compatibility matrix, migrate document shape before component values, reject unknown or over-budget data on strict paths, and have historical/current golden fixtures.
...
- R31. Every public or persistent breaking change must ship an English migration note that names the removed contract, replacement, affected examples/fixtures, and any one-time cache or source migration; release notes may describe deliberate incompatibility but must not preserve obsolete code.
...
- AE7. Given every supported legacy version fixture, when it is loaded stepwise or through a composed migration, then both paths produce the same current candidate before component decoding and any failure leaves source bytes and runtime state unchanged.
...
- KTD9. **Migrate the envelope before payloads.** Domain-neutral envelope/version values live below format owners, while each kind owns pure migrations and a compatibility matrix. Readers run every required document migration before component migration/validation; direct composed and stepwise migration must agree, and runtime loading never rewrites sources.
...
- Persistent source files and artifact metadata will migrate to a common envelope; golden fixtures become compatibility evidence.
...
| Persistent migration corrupts early project files | Add old/current golden fixtures first, migrate into owned candidates, and never rewrite sources during load. |
...
| U8 | Stable runtime identity | `crates/nara_scene/`, `crates/nara_gameplay/`, `crates/nara_reflect/` | U3, U4 |
...
- **Dependencies:** U2.
- **Files:** `docs/architecture/adr/0052-task-backpressure-cancellation-and-long-running-diagnostics.md`, `crates/nara_core/src/limits.rs`, `crates/nara_core/src/lib.rs`, `crates/nara_tasks/src/lib.rs`, `crates/nara_project/src/sections.rs`, `crates/nara_project/src/profile.rs`, `crates/nara_project/src/effective.rs`, `crates/nara_project/src/tests.rs`, `src/lib.rs`.
- **Approach:** Introduce only unit-safe non-zero limit scalars in `nara_core`; keep task queue policy and outcomes in `nara_tasks`. Use bounded submission channels, catch task panics into failed terminals, resolve cancellation/result races by terminal state, define finite cancel/drain/join shutdown, track queue age and failures, and keep an explicitly named inline test executor. Submission returns monotonic `TaskId` plus an explicit domain key; workers produce typed terminal handles only. Domain plugins poll in their existing `TaskUpdateSet::Poll` stage, sort ready results by `(admission_tick, domain_key, task_id)`, and apply in their declared Apply stage without a type-erased global result bus.
...
- **Requirements:** R8 and R12.
- **Dependencies:** U3, U4.
- **Files:** `docs/architecture/adr/0058-stable-runtime-identity-and-entity-references.md`, `crates/nara_scene/src/document.rs`, `crates/nara_scene/src/spawn.rs`, `crates/nara_scene/src/export.rs`, `crates/nara_scene/src/tests.rs`, `crates/nara_gameplay/src/lib.rs`, `crates/nara_reflect/src/value.rs`, `crates/nara_reflect/src/schema.rs`, `src/lib.rs`.
- **Approach:** First implement the Identity Invariants table as a narrow two-world fork, duplicate-scene, unload/reload, and tombstone spike. Then add a world identity-domain allocator/index, make scene instance allocation domain-global, represent structured entity references in reflected values, distinguish scene-local and persistent runtime targets, and define deterministic remap/tombstone behavior for spawn, clone, fork, unload, and lookup.
...
- **Test scenarios:** Multiple spawners cannot collide; two instances of the same scene have distinct targets; clone/fork remaps internal references; missing/tombstoned reference is diagnostic; serialization contains stable reference data but no `Entity`; command lookup resolves through the world index.
...
- **Goal:** Implement conservative schema capabilities, registry freeze, bounded strict decoding, migrations, and golden persistent formats.
...
- **Approach:** Introduce domain-neutral envelope/version values in `nara_core`, strict bounded readers, a compatibility matrix and per-kind migration registries, field-path migration context, explicit capabilities and freeze, field-aware export/apply gates, and owned candidate documents/world mutations that publish only after full success. Each later format-owning unit must add its own historical/current fixtures before enabling the new shape.
- **Execution note:** Add legacy/current golden fixtures and failure-atomicity tests before changing serialized shapes.
...
- **Test scenarios:** Every supported `kind x version` edge; stepwise and composed migration equivalence; missing migration edge; unsupported future version; unknown field rejection; byte/depth/count limits; field path rename; mixed-capability field export/apply rejection; registry mutation after freeze; encode/apply failure leaves source/target unchanged; golden JSON/RON canonical roundtrip.
- **Verification:** AE7 passes and all persisted document kinds have envelope/migration fixtures before their new format is used by examples.
...
- **Files:** `docs/architecture/adr/0036-event-message-and-resource-queue-lifetime.md`, `docs/architecture/adr/0049-untrusted-project-input-and-parse-budget-policy.md`, `docs/architecture/adr/0050-asset-root-symlink-junction-and-package-trust-policy.md`, `docs/architecture/adr/0060-asset-source-vfs-trust-and-rename.md`, `crates/nara_asset/src/identity.rs`, `crates/nara_asset/src/database.rs`, `crates/nara_asset/src/reload.rs`, new focused source modules under `crates/nara_asset/src/`, `crates/nara_asset/tests/filesystem_source.rs`, `crates/nara_asset_watch/src/lib.rs`, `crates/nara_project/src/` budget/profile modules, asset metadata fixtures under `tests/fixtures/`.
...
- **Dependencies:** U5, U11.
- **Files:** `docs/architecture/adr/0037-asset-load-request-cache-and-lifetime-policy.md`, `docs/architecture/adr/0061-import-dependencies-artifact-compatibility-and-readiness.md`, `crates/nara_asset/src/import.rs`, `crates/nara_asset/src/reload.rs`, `crates/nara_image/src/lib.rs`, related tests and import fixtures.
...
- **Files:** `docs/architecture/adr/0047-editor-workspace-and-scene-document-state.md`, `docs/architecture/adr/0064-editor-document-io-transactions-and-recovery.md`, `crates/nara_tooling/src/workspace.rs`, `crates/nara_tooling/src/inspector.rs`, `crates/nara_tooling/Cargo.toml`, `src/lib.rs`, editor fixtures.
...
- **Dependencies:** U11.
- **Files:** `docs/architecture/adr/0060-asset-source-vfs-trust-and-rename.md`, `crates/nara_asset/src/database.rs`, `crates/nara_asset/src/reload.rs`, a focused rename/reconciliation module, `crates/nara_asset_watch/src/lib.rs`, and restart fixtures/tests.
...
- **Files:** `docs/architecture/adr/0061-import-dependencies-artifact-compatibility-and-readiness.md`, `crates/nara_asset/src/artifact.rs`, focused publication/cache modules, artifact fixtures, and integration tests.
...
- **Files:** `docs/architecture/adr/0037-asset-load-request-cache-and-lifetime-policy.md`, `docs/architecture/adr/0061-import-dependencies-artifact-compatibility-and-readiness.md`, `crates/nara_asset/src/state.rs`, `crates/nara_asset/src/storage.rs`, `crates/nara_asset/src/reload.rs`, `crates/nara_image/src/lib.rs`, render preparation consumers, and tests.
...
- **Files:** `docs/architecture/adr/0064-editor-document-io-transactions-and-recovery.md`, `crates/nara_tooling_fs/Cargo.toml`, `crates/nara_tooling_fs/src/lib.rs`, focused platform modules, `crates/nara_tooling_fs/tests/filesystem_transactions.rs`, root `Cargo.toml`, `Cargo.lock`, and `src/lib.rs` advanced exports.
...
- **Files:** `docs/architecture/adr/0064-editor-document-io-transactions-and-recovery.md`, `crates/nara_tooling/src/workspace.rs`, `crates/nara_tooling_fs/src/lib.rs`, focused journal/lock modules, recovery fixtures, and integration tests.
...
| `cargo check --workspace --features serde` | U9-U12, U15, U17, U23, U26-U30, U20 | All persistent/document/profile/intent/artifact/journal/export shapes compile with serialization. |
...
- Persistent readers are bounded and migration-aware; failure paths do not publish partial world, document, artifact, or workspace state.
