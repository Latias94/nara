...
- KTD8. **Separate and freeze schema authority.** Rust declarations and persistent data schemas own non-reused type/field identities and tombstones; names are aliases. An immutable runtime-independent catalog is separate from process-local Rust/Bevy/codec bindings, and both freeze per runtime before startup becomes ready. Capability defaults are empty, mutation APIs cannot bypass the frozen authority, and mixed-capability whole-value writes are rejected until a field projection is used. Dynamic non-Rust components and scripting-adapter projections require later evidence.
...
- **Dependencies:** U1, U8.
- **Files:** `docs/architecture/adr/0043-scene-prefab-and-patch-document-migration-policy.md`, `docs/architecture/adr/0045-component-schema-capability-metadata.md`, `docs/architecture/adr/0051-persistent-file-envelope-migration-and-golden-fixtures.md`, `docs/architecture/adr/0081-schema-source-stable-identity-catalog-and-runtime-binding.md`, `crates/nara_core/src/format.rs`, `crates/nara_core/src/lib.rs`, `crates/nara_reflect/src/schema.rs`, `crates/nara_reflect/src/registry.rs`, `crates/nara_reflect/src/value.rs`, `crates/nara_reflect/src/tests.rs`, `crates/nara_scene/src/document.rs`, `crates/nara_scene/src/prefab.rs`, `crates/nara_scene/src/patch.rs`, `crates/nara_scene/src/format.rs`, `crates/nara_scene/src/validation.rs`, `crates/nara_scene/src/export.rs`, `crates/nara_scene/src/spawn.rs`, `crates/nara_scene/src/tests.rs`, `tests/fixtures/`.
- **Approach:** Introduce domain-neutral envelope/version values in `nara_core`, strict bounded readers, and an enumerable compatibility matrix. Split the immutable runtime-independent component catalog from process-local Rust/Bevy/codec bindings; keep `ComponentTypeId` as an opaque permanent ID, add stable non-reused `ComponentFieldId` plus aliases/tombstones, and make the current `ComponentFieldPath` a value locator rather than durable patch identity. Freeze catalog and bindings per runtime through the registry-owner plugin, make capabilities explicit, and gate whole-value export/apply by complete field eligibility. Add optional per-kind migration registries only for explicitly retained versions, migrate component values and stable field targets before validation, and publish only owned candidate documents/world mutations after full success. Delete superseded pre-launch readers/types/fixtures, rename the corrected Rust API to the canonical unsuffixed name, and write the corrected persistent shape as version 1. Each later format-owning unit adds canonical fixtures before enabling its new shape.
- **Execution note:** Add canonical golden fixtures and failure-atomicity tests before replacing serialized shapes; update all in-repo source documents in the same unit rather than adding a compatibility reader. Do not pull dynamic `ComponentId`, `RuntimeSchemaRecord`, generated sidecar tooling, a scripting adapter, VM, or code-patching implementation into U9.
