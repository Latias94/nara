...
execution_state: inactive-needs-rebaseline
...
- **Objective:** Evaluate which renderer and runner extension levels external Rust packages actually need, then admit and implement only the smallest mechanisms proved by independent production-shaped tracers.
- **Authority:** Accepted ADRs 0046, 0048, 0068, 0078, 0079, 0092, and 0094 govern plugin composition, diagnostics, resource budgets, the minimal render execution boundary, compiled product-capability ceilings, and SDR compatibility. Pipeline Family, Recipe, Feature/pass catalog, retained scene, graph/compiler, exact-GPU interop, and replacement Host vocabulary in this plan is candidate design input, not Accepted authority. The active reference-game plan remains the sole current execution contract.
- **Activation gate:** No unit starts while `docs/plans/2026-07-12-001-refactor-reference-game-driven-foundation-plan.md` remains the active registration. The default gate is completion of that whole plan. An earlier unit may start only after a durable engineering-memory handoff records the baseline commit, completed RGF units, new owner, exact transferred files, files still owned by the RGF executor, and handoff time. After either release path, an implementation-read-only audit followed by a reviewed `docs/knowledge/engineering/render-extension-activation-rebaseline.md` must record the actual public symbols, module owners, accepted ADR state, OQ-001/OQ-002/OQ-022 status, already-landed capabilities, file ownership, verification commands, and which candidate experiments are retained, split, reordered, or removed. The plan frontmatter must then be changed to `execution_state: active` before U7 or any other unit begins.
...
Nara will validate renderer extensibility through independently falsifiable external-package experiments. ADR 0094 does not preselect their sequence or public taxonomy: the activation rebaseline may split this plan into focused successors, remove unsupported candidates, and compare static phases, typed providers, and a minimal render execution kernel before any logical graph or Family API is accepted. Passing a lower-authority experiment never substitutes for a higher-authority proof.
...
The product goal is Bevy-level reachable freedom without forcing ordinary authors to learn backend internals. ADR 0094 deliberately does not yet grant a complete set of public render roles. A stock renderer feature alone cannot prove Unity SRP/HDRP-like replacement, raw GPU integration, complete target/submission ownership, or a mature Editor viewport path, but one broad taxonomy-shaped tracer could also bias Nara toward recreating a Bevy-sized public render surface without evidence.
...
- R28. U2 proves materially different stylized/high-fidelity renderer policy with ADR 0092-compatible SDR output. Real HDR/wide-gamut/tone-mapping waits for OQ-021, and shared Editor/process render-execution topology waits for OQ-022; this plan cannot settle either implicitly.
...
- AE11. Under the Accepted OQ-022 execution-ownership model, an isolated Play runtime view and an offscreen Edit/Editor view render concurrently through independent target transactions; cancellation or failed publication on either axis leaves the other axis and its last-good generation coherent.
...
- ADR 0046, ADR 0048, ADR 0049, ADR 0051, ADR 0068, ADR 0078, ADR 0079, ADR 0092, and ADR 0094 remain accepted authority for runtime plugin roles, diagnostics, persistent input, budgets, the minimal renderer/Host baseline, compiled product ceilings, and current SDR output scope. Candidate renderer mechanisms require focused successors.
...
- U2 may begin only after OQ-022 or an Accepted successor selects the Editor/process render-execution ownership model. U2 remains SDR-only unless OQ-021 is separately accepted and deliberately added through a reviewed rebaseline; real HDR is not needed for this parity proof.
...
- KTD5. ADR 0094 supersedes ADR 0077 and accepts only the minimal render execution boundary. Every higher renderer role remains a candidate until a focused tracer compares alternatives and an Accepted ADR admits it.
...
| U2 silently decides Editor render-execution ownership or real HDR | Critical | High | OQ-022 hard dependency, ADR 0092 SDR-only proof, and separate OQ-021 gate |
...
These units are inactive candidate experiments, not an authorized implementation sequence. The
activation rebaseline must reassess each unit against ADR 0094, may split the plan into focused
successors, and must remove any unit whose premise lacks a production-shaped consumer. Terms moved
out of Accepted authority cannot enter production code merely because they appear below.
...
- **Files:** Create the smallest root package module identified by U7's rebaseline, expected under `src/package.rs`, and modify its facade/root owner; modify `crates/nara_render/src/lib.rs` and focused modules under `crates/nara_render/src/`; modify first-party extraction consumers under `crates/nara_sprite_render/src/` and `crates/nara_ui_render/src/`; extend `tests/render_extension_contracts.rs` and `tests/fixtures/render-extension/multi-role-package/`; create separate design and renamed-acceptance fixtures under `tests/fixtures/render-extension/portable-feature/`; update ADR 0046, ADR 0079, ADR 0094 or a focused successor, the package/composition Interface designs, `docs/architecture/render-extension-capability-interface-design.md`, and `docs/architecture/adr/implementation-status.md` according to proven evidence.
...
- **Patterns to follow:** `tests/derive_dependency_fixtures.rs`, `tests/fixtures/derive-dependencies/renamed-root/`, first-party extraction modules, ADR 0094's owned frame boundary, and candidate provider/packet alternatives in the render Interface harness.
- **Test scenarios:**
...
- **Files:** Modify only the render-execution owners selected by the activation experiment; modify `crates/nara_render/src/pass_plan.rs` only to retire or narrow its transitional responsibility; extend the focused external fixture; update ADR 0094 or a focused successor, `docs/architecture/render-extension-capability-interface-design.md`, and `docs/architecture/adr/implementation-status.md` according to proven evidence.
...
- **Patterns to follow:** ADR 0094's minimal boundary, current `crates/nara_render/src/pass_plan.rs`, stock encoder/submission ownership under `crates/nara_render_wgpu/src/`, and the candidate recipe/template/frame-plan design only as one alternative.
- **Test scenarios:**
...
- **Dependencies:** U1, U8, an Accepted OQ-002 material/shader decision, an Accepted OQ-022 decision or successor selecting the Editor/process render-execution ownership model, and an activation rebaseline that identifies a real first-party material/lighting/visibility baseline plus a production-shaped second Family candidate. A test-only renderer invented solely to satisfy this unit is not sufficient.
- **Files:** Modify only the Family/profile/recipe, shader/material, view/history, realization, and tooling owners selected by the activation experiment; extend scene/component-codec round-trip tests without moving render policy into `nara_scene` or `nara_reflect`; create separate design and renamed-acceptance fixtures under `tests/fixtures/render-extension/stylized-family/`; update ADR 0051 golden-format evidence, ADR 0094 or focused successors, `docs/architecture/render-extension-capability-interface-design.md`, and `docs/architecture/adr/implementation-status.md` according to proven evidence.
- **Approach:** Build an external stylized Family with materially different material, lighting, visibility, and frame topology assumptions while retaining ADR 0092-compatible SDR output. Feed it through handwritten and generated shader-program artifacts plus one Material Technique compatibility path. Its adversarial workload combines persistent temporal history, frame-dependent topology, and one declared auxiliary-view, cross-view, or GPU-driven visibility dependency. Land the first ADR 0049/0051-compliant persistent Pipeline Recipe, make the package-owned Renderer Profile seed or select that Recipe, lower the `nara.toml` default through `nara_project`, and persist per-view Recipe overrides as stable semantic references through the render-owned component codec. Two logical/offscreen views share one admitted device domain while selecting stock and external Families independently. Isolated Play and Edit/Editor attempts then follow the Accepted OQ-022 execution-ownership model, own independent publication axes and target transactions, and share only permitted immutable generation/epoch-stamped leases. Expose SDR final color, overlay composition, one picking strategy, and optional semantic outputs through backend-neutral descriptors.
...
- **Patterns to follow:** The admitted result of U8 or its successor, ADR 0049, ADR 0051, ADR 0094, and candidate scenarios RE-20 through RE-23 and RE-50 through RE-53 in `docs/architecture/render-extension-capability-interface-design.md`.
- **Test scenarios:**
  ...
  - Two logical/offscreen views in one admitted device domain independently select stock and external Families through the same catalog without turning Family selection into a process-global singleton; Edit/Play process ownership separately follows OQ-022.
...
- **Files:** Modify only the capability, exact-GPU, epoch, and submission owners selected by the admitted interop experiment; extend the focused external fixtures and backend tests; update ADR 0078, ADR 0094 or a focused interop successor, `docs/architecture/render-extension-capability-interface-design.md`, and `docs/architecture/adr/implementation-status.md` according to proven evidence.
...
- **Files:** Modify only the Host/execution owners selected by the admitted replacement workflow; create the focused conformance and renamed-dependency fixtures; update ADR 0078, ADR 0094 or a focused replacement-Host successor, the accepted runtime/Host ownership ADR, `docs/architecture/render-extension-capability-interface-design.md`, and `docs/architecture/adr/implementation-status.md` according to proven evidence.
...
- **Execution note:** Prove exclusive authority and failure cleanup against the admitted non-stock workflow before adding convenience helpers or multiple Host candidates to normal authoring surfaces. If no such workflow exists, keep U4 inactive or remove it from the activated successor plan. A recording/fault double alone admits neither a public replacement role nor an accepted private conformance guarantee.
