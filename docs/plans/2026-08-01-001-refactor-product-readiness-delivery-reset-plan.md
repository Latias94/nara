...
- KTD6. **Measure the product, not the evidence protocol.** Freeze the existing U9 collector. Add the smallest observation at the owning product seam for each missing metric and remove it when it would otherwise become general infrastructure.
...
| Simplified candidate tests miss a security regression | Test the retained semantic invariants and one mutation per invariant; use `actionlint` for YAML validity instead of mirroring every step. |
...
| U2 | Root `ci_policy` and `artifact_package_policy` tests; candidate workflow `actionlint`; deleted-consumer search; candidate policy mutation coverage. |
