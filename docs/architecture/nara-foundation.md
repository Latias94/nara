...
The crate/module edges below summarize current ownership. RGD-U7 independently accepted ADR 0082
...
flowchart TD
    ...
    App --> Hierarchy[nara_hierarchy: non-linked runtime structure]
    App --> Scene[nara_scene: scene documents + materialization]
...
| `nara_fs` | Host-issued `DirectoryCapability`/`FileCapability`, checked `limit + 1` bounded reads, validated relative components, scoped live-object identity, digest/lock/temp/replace/sync primitives and non-cloneable typed guarantee receipts | Windows handle-relative NT opens/rename, Linux `openat2`, fail-closed proof tiers, and no authorization-bearing raw paths; replacement evidence distinguishes bytes written through the temporary capability from identity-bound bytes observed after publication; unsupported platform primitives remain explicit |
| `nara_ecs` | `bevy_ecs` re-export boundary: `World`, `Entity`, `Component`, `Resource`, `Bundle`, `Commands`, `Query`, `Schedule`, `ScheduleLabel`, and `SystemSet`; advanced lifecycle-free insertion/despawn preparation | Product-facing ECS conventions over `bevy_ecs`, facade-safe derive exports for root-only and renamed dependencies, and private/version-coupled hook/observer plus non-linked relationship probes used only to reject transactions that cannot remain atomic |
...
| `nara_identity` | `WorldIdentityDomain`, `WorldIdentityDomainId`, `SceneInstanceId`, `PersistentRuntimeId`, structured entity references, tombstones, and remaps | World-scoped runtime claims/indexes, atomic spawn/fork/restore identity transactions, lookup validation, retirement, stable non-`Entity` observation vocabulary, and workspace-internal prepared Scene transactions excluded from the root facade |
| `nara_hierarchy` | One Nara-owned non-linked runtime parent relation, relationship-maintained read-only reverse projection, query-oriented access, construction-only writers, and a semantic completion boundary | Bevy relationship maintenance, dirty-generation validation with retained scratch, and provisional construction internals; no persistent schema, lifecycle ownership, UI layout, transform math, sibling order, reparent API, or general provider Interface |
| `nara_transform` | Authored mutable `Transform2d`, opaque immutable `GlobalTransform2d`, and semantic startup/fixed/PostUpdate/Extract completion for the current hierarchy generation; accepted ADR 0097 ceiling: separate 2D/3D base TRS, optional typed post-affine residuals, and opaque derived global affines | Retained Transform-only adjacency and candidate globals; one allocation-free participant change-tick scan at each completion point, O(T + Et) propagation only when dirty, and atomic generation-token publication; later post-affine, exact reparent lowering, 3D propagation, and domain admission remain evidence-gated |
...
| `nara_scene` | Bounded `SceneDocument`/`PrefabDocument` candidates, stable parent facts, `ScenePatchDocument`, `SceneAuthoringSession`, `PrefabSourceResolver`, `SceneEntityId`, `Name`/`Visibility` schemas, scene materialization/export, and documented direct use with caller-owned `nara_reflect::ComponentRegistry` and `bevy_ecs::World` | Asset-aware validation, patch transactions, undo/redo, prefab expansion, one-way lowering into `nara_hierarchy`, failure-atomic exact-membership replacement/unload, and rejection of runtime topology export; no runtime relation ownership or transform re-export |
| `nara_render` | `Camera2d`, `RenderTarget`, `ViewportRect`, completed-global `ExtractedView`, owned topology-only `RenderFramePacket`, `RenderFrame`, static `RenderPassPlan`, `RenderBackendStatus`, `RenderPhaseLabel` | Backend-neutral render-domain data and the admitted generation-stamped one-window/one-target/one-view topology; current camera extraction accepts completed translation and rejects unsupported global linear state; stock pass order, frame lifecycle, backend state, skipped-frame reason, last error, and render resource lifetime vocabulary; no arbitrary external pass/phase promise |
...
| `nara_sprite_render` | `ExtractedSprites`, `QueuedSpriteItems`, `SpriteBatches`, `SpriteMaterialKey`, `TextureUvRect`, `SpriteRenderPlugin` | Completed-global affine sprite extraction, tilemap lowering, failure-atomic retained frame scratch, deterministic sort keys, and material-keyed textured quad batches |
...
Host-start, candidate-publication, driver-selection, or close protocol. ADRs 0082/0084 own those
accepted bounded flows; Editor and platform integrations must not infer permission to drive a raw
`App` from this abbreviated frame view.
...
- `nara_app::RuntimeCandidate` admits one sealed unstarted App with no raw runner and owns every
  ...
  action. RGD-U7 accepted ADR 0084 for this already-compiled, Host-trusted path while retaining
  its explicit limits on universal Host/Runner abstractions and project-data activation.
...
- The root `HeadlessRun` action is the first ordinary file-backed runtime entry. It accepts one
  ...
  headless path through a complete game loop and CLI. RGD-U7 accepted the compatible ADR 0082/0084
  pair for concrete Hosts; this evidence still does not freeze a universal Host/factory Interface
  or authorize native code from project data.
- The root `DesktopRun` action reuses that private start/publication/retirement owner with a
  ...
  packaging belongs to RGF-U7. This concrete desktop path is accepted ADR 0082/0084 evidence, not
  a public universal process Host or Platform Adapter trait.
...
- `nara_tasks` owns bounded threaded pools, typed terminals, ordered-prefix helpers, physical age
  ...
  and worker state is empty. `nara_app` owns only `CoreStage::TaskUpdate`; `nara_tasks` configures no
  business-domain phases. `nara_asset::AssetTaskUpdateSet` owns the Poll, source-resolution,
  spawn, and apply chain. Task consumers capture one task-pool-instance completion cutoff at Poll
  entry, so terminals published later wait for the next invocation.
- `nara_fs` accepts host-opened handles rather than ambient paths. Windows strict traversal is handle-bound; Linux uses `openat2`; unsupported mount, reparse, filesystem, replacement-source, directory enumeration, unlink, or rename guarantees fail closed and remain visible in the capability matrix. A replacement receipt is linear and records both the digest accepted through the temporary-file capability and the bounded digest rebound through the published target identity; only equality is verified content evidence.
...
- Root `EditorProjectSession` is the concrete RGF-U17 product Host for one known-schema workspace and
  one Play owner. It owns the paired workspace persistence authority, actual filesystem capability,
  receipt-backed Save/Reopen, dirty-close intent, recipe preparation,
  `RuntimeStartAttempt`, `RuntimeInstance`, control tickets, close evidence, and retirement.
  `nara_tooling` and egui retain only commands, views, observations, and Apply Changes models.
  RGD-U7 accepts this as implementation evidence for ADR 0082/0084 in the bounded Host-trusted
  scope, not as acceptance of a universal Host topology.
...
- `nara_tooling::EditorWorkspace` is the UI-agnostic editor document authority. It owns open scene slots, active document, selection sets, dirty/saved revisions, external reload pending/conflict state, per-document undo/redo, and workspace command reports. Saved state advances only through its matching non-cloneable `EditorPersistenceAuthority` after a consumed `nara_fs::ReplaceReceipt` proves prior/candidate identity, required durability, and matching post-publication content; ordinary workspace/UI code cannot forge the transition. The current policy remains `DetectOnly`, so a non-cooperative write after receipt observation is handled by external-change detection and explicit Reopen rather than a strong CAS claim.
...
- `nara_asset` owns typed importer contracts, bounded source-change coalescing, dependency-aware bounded reload-request scheduling, load generations, asset state transitions, and bounded asset observations. Source changes and requests are admitted by item and retained-byte ceilings; every admitted request either reaches its registered concrete consumer or becomes a diagnosed terminal rejection in the same task-update frame.
- Asset reload scheduling coalesces same-frame source changes by last semantic event, walks each dependent source once, and combines generation checks with expected-version, state-revision, and slot-revision guards before domain apply systems mutate runtime asset state. The first concrete image consumer is registered through a private non-cloneable authority; unsupported kinds and missing consumers cannot remain queued or indefinitely `Loading`.
- Asset source-change, request-admission, and terminal-resolution failures are structured diagnostics rather than discarded errors. `AssetEvents` is an observational bounded queue: overflow suppresses later deltas behind a sticky rescan marker until a consumer rebuilds from all `AssetStates` and explicitly completes the rescan. Asset reload policy preserves last-good typed values on failed reload, records failed first loads without inventing values, rejects stale removals after a same-frame replacement, and keeps GPU objects in backend caches rather than imported artifacts.
...
- `nara_asset_watch` is an optional desktop watcher adapter behind the root `asset-watch` feature.
  It owns `notify`, validates its root against `AssetSourceRoot`, preserves in-root rename sides,
  and translates raw filesystem events into semantic `AssetSourceChange` values without leaking
  watcher types into `nara_asset`. Its callback admits whole batches through non-blocking event and
  retained-byte limits; Poll drains only the prefix published at entry. Overflow, producer
  contention, disconnect, backend/translation failure, downstream source-change admission failure,
  or queue unavailability publishes bounded counters, pressure, and a structured diagnostic, then
  enters sticky `RescanRequired` and stops the watcher. Recovery requires a Host-authorized full
  source rescan and watcher reconstruction.
...
- The implemented RGF-U4 plugin core uses static declarations, repeatable definitions, pure data-
  ...
  do not select providers. RPR-U3/U4 add a gameplay-prelude `ProductRecipe` for replayable
  ...
  share the same contribution identity/fingerprint checks. The reference game uses one recipe for
  headless, desktop, and Editor paths, while an independently locked renamed-root fixture proves a
  runtime-only plugin plus configured schema contribution through the public Host facade. Advanced
  raw definitions remain only for explicit embedding probes and fault tests. Package installation,
  file-backed plugin settings, and missing-package restoration remain OQ-045. Default groups remain
  explicit product bundles, and `MinimalPlugins` stays headless/minimal. See ADR
  [0046](adr/0046-plugin-metadata-and-default-plugin-groups.md) and the [Rust product recipe
  guide](../guides/rust-product-recipes.md).
