...
- `nara_asset_watch` is an optional desktop watcher adapter behind the root `asset-watch` feature.
  ...
  contention, disconnect, backend/translation failure, or queue unavailability publishes bounded
  counters, pressure, and a structured diagnostic, then enters sticky `RescanRequired` and stops
  the watcher. Recovery requires a Host-authorized full source rescan and watcher reconstruction.
