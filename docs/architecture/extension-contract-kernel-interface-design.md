...
| `ContractSlice<C>` | Bevy `PluginGroupBuilder` ordered live plugin entries before `finish` mutates `App` are the nearest planning analogy | A filtered registry of one specialized plugin kind | Target/platform-selected assembly and attribute candidates | Target-selected module/provider candidates | It contains immutable declarations and semantic binding-presence witnesses, not implementation provenance or callable factories |
