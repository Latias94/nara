...
SRP/HDRP-level complete renderer policy, raw GPU/native integration, mature Editor viewport support,
and whole renderer/runner replacement without giving every ordinary runtime plugin hidden global
authority.
...
> control up to the complete Render Host or Platform/Runner boundary.
...
Nara needs six capability levels, but most users should see only `Plugin` and one renderer choice:

| Capability level | Closest mature-engine concept | Cardinality | Authority gained |
|---|---|---:|---|
| Runtime Plugin | Bevy `Plugin` | Many | ECS types, resources, systems, queues, schedules, and runtime-local domain state |
| Render Feature/Pass | Bevy render systems/graph nodes; Unity Renderer Feature or HDRP Custom Pass; Godot `CompositorEffect` | Many | Typed extraction/packet data, queue policy, declared graph work, and scoped encoding |
| Pipeline Family | Unity `RenderPipelineAsset` plus `RenderPipeline`; Bevy custom camera render schedule | Many registered, one selected per recipe/view | Complete logical renderer policy: material, lighting, visibility, feature compatibility, and frame topology |
| Wgpu/Native Interop | Bevy `RenderDevice`/`RenderQueue`; Unity native graphics plug-in | Many Host-scheduled sessions | Exact GPU/native APIs and persistent device-epoch resources inside the selected Host |
| Render Host Adapter | Bevy `RenderCreation::Manual` injects GPU resources only; full Bevy ownership also requires replacing/omitting `RenderPlugin`. Godot `RendererCompositor` is the closer authority analogy. | One per device domain | Device/Queue, target acquire, encode/submit/present, recovery, placement, and teardown |
| Platform/Runner Adapter | Bevy runner/Winit; Godot `MainLoop` and `DisplayServer` | One per driver scope | Event loop, platform events/time/wake/close, and declared platform-affine authority |

These levels are not six runtime layers and do not execute through a per-frame registry. They are
composition-time and lifecycle ownership choices. A typical game uses the default Host and runner.
A typical SRP/HDRP-like renderer package uses a Pipeline Family, Features, and perhaps interop; it
does not need a custom event loop.
...
`RenderDevice` and `RenderQueue` available as renderer resources. This creates excellent local Rust
leverage. It also means ordering, process ownership, device initialization requirements, and
renderer mutation are often coordinated by plugin order and live resource access.
...
7. External Platform/Runner candidates can drive the managed runtime through an explicit root
   choice.
...
9. The ordinary user enables a coherent renderer through one package/product-preset action and one
   renderer/recipe selection.
...
| RE-02 | Game selects an external complete renderer | One package/product-preset action plus one renderer/recipe choice; no internal role registration | Clean-room author task |
| RE-03 | One project uses different recipes per view | Each view selects one compatible family/recipe without replacing the device Host | Multi-view semantic test |
| RE-04 | Dedicated server uses a package that also contains renderer roles | Renderer, window, interop, and Editor dependencies are absent from the server closure | Cargo tree and runtime resource audit |
...
| ID | Caller and goal | Required behavior | Primary oracle |
|---|---|---|---|
| RE-20 | External package ships an HDR-like renderer | Family owns material/lighting/view assumptions and a different full logical topology | Clean-room HDR-like family tracer |
...
| RE-43 | External platform or runner drives the product | It drives `RuntimeInstance` through public time/event/control/close contracts | Alternate-runner clean-room fixture |
...
| ID | Caller and goal | Required behavior | Primary oracle |
|---|---|---|---|
| RE-50 | Editor displays an external family | Family exposes final SDR/HDR color meaning and an overlay-composition point | Editor viewport tracer |
...
| RE-53 | Runtime and Editor render at once | Each consumer uses coherent immutable family/template/epoch generations and independent target transactions | Edit/Play multi-view test |
...
flowchart TD
    ...
    Runner[One selected Platform/Runner Adapter]
...
- one Platform/Runner owns one driver scope;
...
An advanced package-author helper may conceptually aggregate roles like this:
...
pub fn package() -> Result<PackageDefinition, PackageAuthorReport> {
    package::define(
        generated::PACKAGE,
        (
            nara_render::package::family(generated::FAMILY, family_definition),
            nara_render::package::features(generated::FEATURES, feature_definitions),
            nara_render_wgpu::package::interop(generated::INTEROP, interop_definition),
        ),
    )
}
```

Names and tuple shape are provisional. The helper must hide contract keys, binding receipts, device
request fingerprints, epoch session construction, and Host publication.
...
- typed packet sections and queue/material techniques it consumes;
...
1. a final SDR or HDR color target contract with color-space/transfer meaning;
...
## Platform And Runner Contract

The external runner guarantee is result-level now even though the exact trait split is deferred:
...
Display/window ownership and runtime driving may become separate concrete Adapters, as Godot splits
`DisplayServer` and `MainLoop`. This document does not freeze one oversized trait. The direct
code-first `App::set_runner` plus `App::run` path remains valid but is mutually exclusive with
admission of that App into the managed runtime path.
...
| Platform/Runner replacement | Usually new executable/outer Host generation | Runtime and parent authorities follow stop-first close rules |
...
| Complete Family | HDR-like topology/material/lighting policy, recipe selection, editor final color/overlay/picking | No `nara_render` family match or first-party-only compiler input |
...
|---|---:|---|
| Ordinary author cost | One renderer selection and ordinary gameplay Plugin path | RE-01/RE-02 task study |
...
| Headless isolation | Server closure contains no unselected render/window/interop/Editor dependencies | RE-04 audit |
...
4. Frame packets have an open typed bounded extension channel.
5. Device requirements close before `request_device`.
6. Scoped encoding, wgpu/native interop, and Render Host ownership are separate permission levels.
7. Interop state and all GPU work are bound to one Host/device epoch.
8. Exactly one Render Host owns each device domain and exactly one runner owns each driver scope.
9. Editor-compatible families expose final color/overlay plus one picking strategy; optional
   semantic outputs are explicit capabilities.
10. Ordinary users select a renderer package rather than assemble its internal roles.
11. Direct raw-App runner and managed-runtime runner paths are mutually exclusive.
12. Wgpu remains the only RHI; exact native interop does not enter portable project data or preludes.

## Decisions To Defer

| Decision | Evidence trigger |
|---|---|
| Exact Pipeline Family trait/factory/catalog Rust shape | First external HDR-like family tracer |
...
| Platform display/window versus runtime-driver trait split | Alternate runner plus the later docking/multi-viewport tracer |
...
- [Unity URP Renderer Features](https://docs.unity3d.com/6000.0/Documentation/Manual/urp/renderer-features/intro-to-scriptable-renderer-features.html)
